...
void space(const uint16_t i_usec)
{
pwm_poweroff(PWM_DEV(dev_use));
xtimer_usleep(i_usec);
}

void mark(const uint16_t i_usec)
{
pwm_poweron(PWM_DEV(dev_use));
xtimer_usleep(i_usec);
}
...
static int _ir_send_test(int argc, char** argv)
{
    if (argc != 2) {
        printf("usage: %s <dev>\n", argv[0]);
        printf("\tdev: device by number between 0 and %d\n", PWM_NUMOF - 1);
    ...
    dev_use = _get_dev(argv[1]);
    if (dev_use == UINT_MAX) {
        return 1;
    }

    uint16_t aBufferWithMicroseconds[]={1600,600,600};
    sendRaw(aBufferWithMicroseconds, sizeof(aBufferWithMicroseconds)/sizeof(uint16_t) );
...
static const shell_command_t shell_commands[] = {
    ...
    { "ir_send_test", "IR remote send, test", _ir_send_test },
    { NULL, NULL, NULL }
};
