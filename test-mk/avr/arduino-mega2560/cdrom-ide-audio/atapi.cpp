...
const uint8_t HeadReg = 0xF6;         // Addr. Device/Head register of IDE device. aka "Drive Select"
const uint8_t ComSReg = 0xF7;         // Addr. Command/Status (wr/rd) register of IDE device. aka "ATAPI Status" (p69)
const uint8_t AStCReg = 0xEE;         // Addr. Alternate Status/Device Control (rd/wr) register of IDE dev
...
void atapi::identify_drive(struct identify_drive_details *drive_details)
{
  ...
  /*  Spec seems to suggest that all CD-ROM drives use 12 uint8_t packets? (p78) so not bothering to support 16 uint8_t packets
    if (cnt == 0)                                // Get supported packet lenght
    {
      if(read_val.low & (1<<0))                      // contained in lower uint8_t of first word
      {
...
void atapi::read_subchannel(struct sub_channel_data *data)
{
  ...
  do
  {
    ...
    if (cnt > 16)
    {
      //Serial.print(F("Get sub chan data: ERROR >16 uint8_ts received, but DRQ still set. reg="));
...
void atapi::send_packet(uint8_t *packet, uint8_t packet_length)
{
  uint8_t dataLval;
  uint8_t dataHval;

  _ide->write(AStCReg, 0b00001010, 0xFF);      // Set nIEN before you send the PACKET command!
