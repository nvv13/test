...
const uint8_t ErrFReg
    = 0xF1; // Addr. Error/Feature (rd/wr) register of IDE device.
...
const uint8_t HeadReg
    = 0xF6; // Addr. Device/Head register of IDE device. aka "Drive Select"
const uint8_t ComSReg = 0xF7; // Addr. Command/Status (wr/rd) register of IDE
                              // device. aka "ATAPI Status" (p69)
const uint8_t AStCReg = 0xEE; // Addr. Alternate Status/Device Control (rd/wr)
                              // register of IDE dev

atapi::atapi (ide_io *ideio)
{
  _ide = ideio;
...
atapi::reset ()
{
  ...
  _delay_ms (3000);  // This _delay_ms waits for the drive to initialise
  BSY_clear_wait (); // The ATAPI spec. allows drives to take up to
  DRY_set_wait ();   // 31 sec. but all tested where alright within 3s.
}

bool
atapi::is_atapi_device ()
{
  ide_io::data16 read_val
      = _ide->read (CylLReg); // Check device signature for ATAPI capability

  if (read_val.low == 0x14) // See "5.18.3 Special Handling of ATA Read and
                            // Identify Drive Commands", p61
    {
...
void
atapi::init_task_file ()
{
  _ide->write (ErrFReg, 0x00,
               0xFF); // Set Feature register = 0 (no overlapping and no DMA)
  _ide->write (CylHReg, 0x02,
               0xFF); // Set PIO buffer to max. transfer length (= 200h)
  _ide->write (CylLReg, 0x00, 0xFF);
  _ide->write (AStCReg, 0x02,
               0xFF); // Set nIEN, we don't care about the INTRQ signal
...
void
atapi::identify_drive (struct identify_drive_details *drive_details)
{
  uint8_t cnt = 0;
  uint8_t model_idx = 0;
  ide_io::data16 read_val;
  ...
  _ide->write (ComSReg, 0xA1, 0xFF); // Issue Identify Device Command     "0xA1
                                     // = ATAPI Identify Device". 7.1.7, p76
  _delay_ms (500); // Instead of wait for IRQ. Needed by some dev.
  do
    {
      read_val = _ide->read (DataReg);
      /*  Spec seems to suggest that all CD-ROM drives use 12 uint8_t packets?
        (p78) so not bothering to support 16 uint8_t packets if (cnt == 0) //
        Get supported packet lenght
        {
          if(read_val.low & (1<<0))                      // contained in lower
        uint8_t of first word
          {
            paclen = 16;                              // 1st bit set -> use 16
        uint8_t packets
          }
        } */
...
void
atapi::request_sense (struct sense_data *sd)
{
  ide_io::data16 read_val;
  uint8_t packet[12] = { 0 };
  ...
  if (track == 0xFF)
    {
      ...
      // The spec suggests that sending M/S/F all set to 0xFF here should work,
      // but it doesn't seem to... so get the current location and send that
...
void
atapi::read_subchannel (struct sub_channel_data *data)
{
  uint8_t packet[12] = { 0 };
  ide_io::data16 read_val;
  ...
  do
    {
      ...
      if (cnt > 16)
        {
          printf ("Get sub chan data: ERROR >16 uint8_ts received, but DRQ "
                  "still set. reg=%d\r\n",
                  read_val.low);
          break;
        }
...
atapi::read_toc ()
{
  ...
  // Get TOC Track Descriptors: descriptor is 8 uint8_ts (so 4 x 16bit reads)
  read_val = _ide->read (ComSReg);
  while (read_val.low & (1 << 3)) // Read data from DataRegister until DRQ=0
    {
      read_val
          = _ide->read (DataReg); // reserved, ADR & control (don't care. yet.)
      ...
      if (_toc.track[toc_idx].track_number
          == 0xAA) // "A track number of 0AAh indicates that the track
      ...
      if (toc_idx < MAX_TOC_LENGTH)
        ...
        {
          printf (
              "Read TOC: reached MAX_TOC_LENGTH, but DRQ still 0.reg=%d\r\n",
              read_val.low);
          break;
        }
...
atapi::mode_sense_media_status ()
{
  ...
  read_val = _ide->read (DataReg); // get Medium Type (see "Table 59 - CD-ROM
                                   // Media Type Codes", p123)
...
void
atapi::send_packet (uint8_t *packet, uint8_t packet_length)
{
  uint8_t dataLval;
  uint8_t dataHval;

  _ide->write (AStCReg, 0b00001010,
               0xFF); // Set nIEN before you send the PACKET command!
  _ide->write (ComSReg, 0xA0, 0xFF); // Write Packet Command Opcode

  for (int idx = 0; idx < packet_length;
       idx += 2) // Send packet with length of 'paclen'
...
bool
atapi::get_track_address (uint8_t track, struct msf_address **track_addr)
{
...
atapi::MediaStatus
atapi::decode_media_type_code (uint8_t status)
{
  if (status == 0x71)
    return atapi::MediaStatus::DOOR_OPEN;
  if (status == 0x70)
    return atapi::MediaStatus::DOOR_CLOSED_NO_DISC;
  if (status == 0x00) // Some CD-ROM drives seem to report this for audio (and
                      // probably any?) disc types
