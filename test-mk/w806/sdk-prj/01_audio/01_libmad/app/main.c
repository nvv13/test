...
user_task (void *sdata)
{
  ...
  tls_sys_clk_set (CPU_CLK_240M); // нам мужно 240MHz
...
output (void *data, struct mad_header const *header, struct mad_pcm *pcm)
{
  ...
  if (stat_dma == D_NONE)
    {
      i2s_DMABUF_SIZE = nsamples * 2 * nchannels;
      i2s_DMABUF = malloc (i2s_DMABUF_SIZE * 2);
      if (i2s_DMABUF == NULL)
        {
          printf ("error malloc i2s_DMABUF\r\n");
          return MAD_FLOW_BREAK;
        }
      mp3_BUF = malloc (i2s_DMABUF_SIZE);
      if (mp3_BUF == NULL)
        {
          free (i2s_DMABUF);
          i2s_DMABUF = NULL;
