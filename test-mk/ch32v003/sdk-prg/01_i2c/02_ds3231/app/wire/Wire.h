...
extern "C"
{
  ...
  typedef enum
  {
    ...
    // не для w801 I2C_SPEED_FAST_PLUS = 1000000,    /**< fast plus mode:
    // ~1Mbit/s */ не для w801 I2C_SPEED_HIGH = 3400000,         /**< high
    // speed mode: ~3.4Mbit/s */
  } i2c_speed_t;
  ...
  typedef struct
  {
    pin_t i2c_scl; /* WM_IO_PA_01 or WM_IO_PB_20 */
    pin_t i2c_sda; /* WM_IO_PA_04 or WM_IO_PB_19 */
    u32 i2c_freq; /* частота i2c в герцах, если = 0 - то софтверный вариант,
                     любые gpio можно использовать */
    bool hw_wire;
  } i2c_param_t;
   ...
   *
   * The given I2C device will be initialized with the parameters as specified
   * in the boards periph_conf.h, using the pins and the speed value given
   * there.
   *
   ...
   * @return                  -ENXIO When no devices respond on the address
   * sent on the bus
   * @return                  -ETIMEDOUT  When timeout occurs before device's
   * response
   * @return                  -EINVAL When an invalid argument is given
   * @return                  -EOPNOTSUPP When MCU driver doesn't support the
   * flag operation
   * @return                  -EAGAIN When a lost bus arbitration occurs
   */

  int i2c_read_reg (i2c_t dev, uint16_t addr, uint16_t reg, void *data,
                    uint8_t flags);
   ...
   * @return                  -EAGAIN When a lost bus arbitration occurs
   */
  int i2c_read_regs (i2c_t dev, uint16_t addr, uint16_t reg, void *data,
   ...
   *          specified register address. It first writes the register then
   * data.
   *
   * @pre     i2c_acquire must be called before accessing the bus
   *
   ...
   * @return                  -EAGAIN When a lost bus arbitration occurs
   */
  int i2c_write_regs (i2c_t dev, uint16_t addr, uint16_t reg, const void *data,
                      size_t len, uint8_t flags);
