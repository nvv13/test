...
static void showNumberBaseEx (int8_t base, uint16_t num, uint8_t dots,
                              uint8_t leading_zero, uint8_t length,
                              uint8_t pos);
...
void
TM1637Display (enum tls_io_name pinClk, enum tls_io_name pinDIO,
               unsigned int bitDelay)
{
  ...
  // Both pins are set as inputs, allowing the pull-up resistors to pull them
  // up
...
void
setBrightness (uint8_t brightness, uint8_t on)
{
...
void
showNumberDec (int num, uint8_t leading_zero, uint8_t length, uint8_t pos)
{
  showNumberDecEx (num, 0, leading_zero, length, pos);
}

void
showNumberDecEx (int num, uint8_t dots, uint8_t leading_zero, uint8_t length,
                 uint8_t pos)
{
  showNumberBaseEx (num < 0 ? -10 : 10, num < 0 ? -num : num, dots,
                    leading_zero, length, pos);
}

void
showNumberHexEx (uint16_t num, uint8_t dots, uint8_t leading_zero,
...
static void
showNumberBaseEx (int8_t base, uint16_t num, uint8_t dots,
                  uint8_t leading_zero, uint8_t length, uint8_t pos)
{
  ...
  for (uint8_t i = 0; i < 8; i++)
    {
      ...
      if (data & 0x01)
        ...
        {
          tls_gpio_cfg (m_pinDIO, WM_GPIO_DIR_OUTPUT,
                        WM_GPIO_ATTR_FLOATING); // 0
          tls_gpio_write (m_pinDIO, 0);
        }
...
static void
showDots (uint8_t dots, uint8_t *digits)
{
  for (int i = 0; i < 4; ++i)
    {
      digits[i] |= (dots & 0x80);
