  ...
  for (int iInd = 0; iInd < i_len; iInd++)
    {
      ...
      switch (u8_status_find)
        ...
        case RG_FIND_PRE2:
          {
            if (ch
                == c_PRE2[i_POS_PRE2]) // "unit_temperature_c"; span
                                       // class="unit unit_temperature_c"><span
                                       // class="sign">&minus;</span>4<span
                                       // class="lower">,3</span>
              i_POS_PRE2++;
            else
              {
        ...
        case RG_FIND_PRE3:
          {
            if (ch == c_PRE3[i_POS_PRE3]) // "sign\">";   ="sign">+</span> ,
                                          // грузим знак, если есть и дальше...
              i_POS_PRE3++;
            else
              {
        ...
        case RG_FIND_PRE4:
          {
            if (ch == c_PRE4[i_POS_PRE4]) // "</span>"; далее идёт число -
                                          // температура  </span>20<span
              i_POS_PRE4++;
            else
              {
        ...
        case RG_FIND_PRE5:
          {
            if (ch == c_PRE5[i_POS_PRE5]) // "<";
              ...
              {
                if (i_POS_PRE5 < strlen (c_PRE5)
                    && u8_buf_pos_temperatura < 10)
                  {
                    i_POS_PRE5 = 0;
                    buf_temperatura[u8_buf_pos_temperatura++] = ch;
                  }
