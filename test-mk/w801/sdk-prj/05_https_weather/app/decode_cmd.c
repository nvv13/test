  ...
  if (strncmp ((char *)sock_rx, "time", 4) == 0)
    {
      ...
      i_len_ret = sprintf (
          ...
          t_last_query.tm_year + 1900, t_last_query.tm_mon + 1,
          t_last_query.tm_mday, t_last_query.tm_hour, t_last_query.tm_min,
  ...
  if (len > 5 && strncmp ((char *)sock_rx, "mode=", 5) == 0)
    {
      ...
      if (i_out > 0 && i_out < 4)
        ...
        i_len_ret = sprintf (sock_rx, "error value=%d, cur i_mode_global=%d",
                             i_out, i_mode_global);
    }
  ...
  if (strncmp ((char *)sock_rx, "help", 4) == 0)
    {
      ...
      i_len_ret = sprintf (
          ...
          "lightXXX - установить/узнать яркость индикатора, где XXX число от "
          "4 до 2000, если 0 то выдаст текущее значение яркости\n"
          "mode=X - режим, где X, только 1-погода, 2-погода и время, 3-только "
          "время\n"
