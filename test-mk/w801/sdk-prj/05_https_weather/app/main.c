...
void demo_console_task(void *sdata)
{
   ...
   for(;;) // цикл(1) с подсоединением к wifi и запросом времени
    ...
    while(u8_wifi_state==1) // основной цикл(2)
     ...
     if(tblock.tm_hour==23 && tblock.tm_min==0 && tblock.tm_sec==0 && i_max_out!=LCD_VAL_LG_spb_hi) // ночь, установить минимальную яркость!
            {
            i_max_out=LCD_VAL_LG_spb_hi;
            }

     if(tblock.tm_hour==6 && tblock.tm_min==0 && tblock.tm_sec==0 && i_max_out==LCD_VAL_LG_spb_hi) // утро, восстановить яркость!
            {
            i_max_out=LCD_VAL_LG_hi;
            }
     ...
     if(i_mode_global!=GL_MODE_CLOCK && 
      ...
      {
      u8_wifi_state=0; // переход на цикл(1) wifi по новой и запрос температуы
      if(my_recognize_ret_cur_temperature()==MY_RECOGNIZE_NO_VALUE)
       {
       cnt_no_value++;
