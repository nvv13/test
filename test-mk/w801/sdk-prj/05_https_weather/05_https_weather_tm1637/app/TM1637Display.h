...
//! data pins.
//!
//! @param pinClk - The number of the digital pin connected to the clock pin of
//! the module
//! @param pinDIO - The number of the digital pin connected to the DIO pin of
//! the module
//! @param bitDelay - The delay, in microseconds, between bit transition on the
//! serial
//!                   bus connected to the display
void TM1637Display (enum tls_io_name pinClk, enum tls_io_name pinDIO,
...
//! function may either set the entire display or any desirable part on its
//! own. The first digit is given by the @ref pos argument with 0 being the
//! leftmost digit. The @ref length argument is the number of digits to be set.
//! Other digits are not affected.
//!
...
//!        are to be displayed, the number must be between 0 to 99)
//! @param pos The position of the most significant digit (0 - leftmost, 3 -
//! rightmost)
void showNumberDec (int num, uint8_t leading_zero, uint8_t length,
                    uint8_t pos);
