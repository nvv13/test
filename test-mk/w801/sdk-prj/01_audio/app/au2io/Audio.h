...
public:
  bool
  begin (SdCsPin_t csPin = SS, uint32_t maxSck = SD_SCK_MHZ (25))
  {
    return SdFat::begin (csPin, maxSck);
  }
};
...
public:
  ...
  usedBytes ()
  {
    // set SdFatConfig MAINTAIN_FREE_CLUSTER_COUNT non-zero. Then only the
    // first call will take time.
    return (uint64_t) (clusterCount () - freeClusterCount ())
...
extern __attribute__ ((weak)) void audio_id3data (const char *); // ID3
                                                                 // metadata
extern __attribute__ ((weak)) void
audio_id3image (File &file, const size_t pos,
                const size_t size); // ID3 metadata image
extern __attribute__ ((weak)) void
audio_eof_mp3 (const char *); // end of mp3 file
...
extern __attribute__ ((weak)) void
audio_eof_stream (const char *); // The webstream comes to an end
extern __attribute__ ((weak)) void
audio_process_extern (int16_t *buff, uint16_t len,
                      bool *continueI2S); // record audiodata or send via BT
extern __attribute__ ((weak)) void
audio_process_i2s (uint32_t *sample,
                   bool *continueI2S); // record audiodata or send via BT
...
class AudioBuffer
{
  ...
  //
  //
  //   if the space between m_readPtr and buffend < m_resBuffSize copy data
  //   from the beginning to resBuff so that the mp3/aac/flac frame is always
  //   completed
  //
...
protected:
  size_t m_buffSizePSRAM
      = 300000; // most webstreams limit the advance to 100...300Kbytes
...
public:
  ...
  bool connecttomarytts (const char *speech, const char *lang,
                         const char *voice);
  ...
  bool setPinout (uint8_t BCLK, uint8_t LRC, uint8_t DOUT,
                  int8_t DIN = I2S_PIN_NO_CHANGE,
                  int8_t MCK = I2S_PIN_NO_CHANGE);
  bool pauseResume ();
  bool
  isRunning ()
  {
...
#define ESP_ARDUINO_VERSION_PATCH 0
  ...
  bool
  startsWith (const char *base, const char *str)
  {
    // fb
    char c;
    while ((c = *str++) != '\0')
  ...
  bool
  endsWith (const char *base, const char *str)
  {
    // fb
  ...
  int
  indexOf (const char *base, const char *str, int startIndex = 0)
  {
    // fb
    const char *p = base;
  ...
  int
  indexOf (const char *base, char ch, int startIndex = 0)
  {
    // fb
    const char *p = base;
    for (; startIndex > 0; startIndex--)
  ...
  int
  lastIndexOf (const char *haystack, const char *needle)
  {
    // fb
  ...
  int
  specialIndexOf (uint8_t *base, const char *str, int baselen,
                  bool exact = false)
  {
    int result; // seek for str in buffer or in header up to baselen, not
                // nullterninated
  ...
  size_t
  bigEndian (uint8_t *base, uint8_t numBytes, uint8_t shiftLeft = 8)
  {
    size_t result = 0;
  ...
  bool
  b64encode (const char *source, uint16_t sourceLength, char *dest)
  {
    ...
    if (buffer)
      {
        ...
        int len = base64_encode_block (&source[0], sourceLength, &buffer[0],
                                       &_state);
  ...
  size_t
  urlencode_expected_len (const char *source)
  {
  ...
  void
  vector_clear_and_shrink (vector<char *> &vec)
  {
    uint size = vec.size ();
...
private:
  const char *codecname[9] = { "unknown", "WAV", "MP3",      "AAC", "M4A",
                               "FLAC",    "OGG", "OGG FLAC", "OPUS" };
  enum : int
  {
    APLL_AUTO = -1,
    APLL_ENABLE = 1,
  ...
  const uint8_t volumetable[22]
      = { 0,  1,  2,  3,  4,  6,  8,  10, 12, 14, 17,
  ...
  WiFiClientSecure
      clientsecure; // @suppress("Abstract class cannot be instantiated")
  ...
  uint32_t m_resumeFilePos
      = 0; // the return value from stopSong() can be entered here
