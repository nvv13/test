...
static my_ESP ESP = my_ESP ();

//---------------------------------------------------------------------------------------------------------------------
AudioBuffer::AudioBuffer (size_t maxBlockSize)
{
  // if maxBlockSize isn't set use defaultspace (1600 bytes) is enough for aac
  // and mp3 player
  if (maxBlockSize)
...
void
AudioBuffer::setBufsize (int ram, int psram)
{
...
void
AudioBuffer::changeMaxBlockSize (uint16_t mbs)
{
  m_maxBlockSize = mbs;
  return;
}
...
AudioBuffer::writeSpace ()
{
  if (m_readPtr >= m_writePtr)
    {
      m_writeSpace
          = (m_readPtr - m_writePtr - 1); // readPtr must not be overtaken
    }
  else
    {
      if (getReadPos () == 0)
...
void
AudioBuffer::bytesWritten (size_t bw)
{
  m_writePtr += bw;
...
void
AudioBuffer::bytesWasRead (size_t br)
{
  m_readPtr += br;
...
uint8_t *
AudioBuffer::getWritePtr ()
{
  return m_writePtr;
}

uint8_t *
AudioBuffer::getReadPtr ()
{
...
//---------------------------------------------------------------------------------------------------------------------
Audio::Audio (bool internalDAC /* = false */,
              uint8_t channelEnabled /* = I2S_DAC_CHANNEL_BOTH_EN */,
              uint8_t i2sPort)
{
  ...
  //    I2S_DAC_CHANNEL_RIGHT_EN = 1,     Enable I2S built-in DAC right
  //    channel, maps to DAC channel 1 on GPIO25 I2S_DAC_CHANNEL_LEFT_EN  = 2,
  //    Enable I2S built-in DAC left  channel, maps to DAC channel 2 on GPIO26
  //    I2S_DAC_CHANNEL_BOTH_EN  = 0x3,   Enable both of the I2S built-in DAC
  //    channels. I2S_DAC_CHANNEL_MAX      = 0x4,   I2S built-in DAC mode max
  //    index
#ifdef AUDIO_LOG
  m_f_Log = true;
#endif
  ...
  clientsecure.setInsecure (); // if that can't be resolved update to ESP32
                               // Arduino version 1.0.5-rc05 or higher
  ...
  if (internalDAC)
    {
      ...
      m_i2s_config.communication_format
          = (i2s_comm_format_t) (I2S_COMM_FORMAT_STAND_I2S); // vers == 2.0.0
#else
      // V2.0.1 ... V2.0.4 will not work
      m_i2s_config.communication_format
          = (i2s_comm_format_t) (I2S_COMM_FORMAT_STAND_MSB); // vers >= 2.0.5
#endif
#else
      m_i2s_config.communication_format
...
#if ESP_ARDUINO_VERSION_MAJOR >= 2
      m_i2s_config.communication_format = (i2s_comm_format_t) (
          I2S_COMM_FORMAT_STAND_I2S); // Arduino vers. > 2.0.0
#else
      m_i2s_config.communication_format = (i2s_comm_format_t) (
...
//---------------------------------------------------------------------------------------------------------------------
esp_err_t
Audio::I2Sstart (uint8_t i2s_num)
{
...
esp_err_t
Audio::I2Sstop (uint8_t i2s_num)
{
  return i2s_stop ((i2s_port_t)i2s_num);
}
//---------------------------------------------------------------------------------------------------------------------
esp_err_t
Audio::i2s_mclk_pin_select (const uint8_t pin)
{
  // IDF >= 4.4 use setPinout(BCLK, LRC, DOUT, DIN, MCK) only,
  // i2s_mclk_pin_select() is no longer needed
...
{
  // I2Sstop(m_i2s_num);
  // InBuff.~AudioBuffer(); #215 the AudioBuffer is automatically destroyed by
  // the destructor
  setDefaults ();
  if (m_playlistBuff)
    {
...
Audio::setDefaults ()
{
  ...
  m_f_tts = false;
  m_f_firstCall
      = true; // InitSequence for processWebstream and processLokalFile
  m_f_running = false;
...
//---------------------------------------------------------------------------------------------------------------------
void
Audio::setConnectionTimeout (uint16_t timeout_ms, uint16_t timeout_ms_ssl)
{
  if (timeout_ms)
...
//---------------------------------------------------------------------------------------------------------------------
bool
Audio::connecttohost (const char *host, const char *user, const char *pwd)
{
  ...
  setDefaults (); // no need to stop clients if connection is established
                  // (default is true)
  ...
  //  AUDIO_INFO("Connect to \"%s\" on port %d, extension \"%s\"", hostwoext,
  //  port, extension);

  char rqh[strlen (h_host) + strlen (authorization)
           + 200]; // http request header
  rqh[0] = '\0';
  ...
  if (ESP_ARDUINO_VERSION_MAJOR == 2 && ESP_ARDUINO_VERSION_MINOR == 0
    ...
    {
      m_timeout_ms_ssl
          = UINT16_MAX; // bug in v2.0.3 if hostwoext is a IPaddr not a name
      m_timeout_ms = UINT16_MAX; // [WiFiClient.cpp:253] connect(): select
                                 // returned due to timeout 250 ms for fd 48
    }
...
//---------------------------------------------------------------------------------------------------------------------
bool
Audio::httpPrint (const char *host)
{
  // user and pwd for authentification only, can be empty
...
Audio::UTF8toASCII (char *str)
{
  ...
  const uint8_t ascii[60] = {
    ...
    // 143, 144, 145, 146, 147, 148  // UTF8(C3)
    //                Ä    Å    Æ    Ç         É Ñ                  // CHAR
    000, 000, 000, 0xC4, 143, 0xC6, 0xC7, 000, 0xC9, 000, 000, 000, 000, 000,
    000, 000, 0xD1, 000, 000, 000, // ASCII (Latin1)
    // 149, 150, 151, 152, 153, 154, 155, 156, 157, 158, 159, 160, 161, 162,
    // 163, 164, 165, 166, 167, 168
    //      Ö                             Ü              ß    à ä    å    æ è
    000, 0xD6, 000, 000, 000, 000, 000, 0xDC, 000, 000, 0xDF, 0xE0, 000, 000,
    000, 0xE4, 0xE5, 0xE6, 000, 0xE8,
    // 169, 170, 171, 172. 173. 174. 175, 176, 177, 179, 179, 180, 181, 182,
    // 183, 184, 185, 186, 187, 188
    //      ê    ë    ì         î    ï         ñ    ò         ô         ö ù û ü
    000, 0xEA, 0xEB, 0xEC, 000, 0xEE, 0xEB, 000, 0xF1, 0xF2, 000, 0xF4, 000,
    0xF6, 000, 000, 0xF9, 000, 0xFB, 0xFC
  };
#else
  const uint8_t ascii[60] = {
    // 129, 130, 131, 132, 133, 134, 135, 136, 137, 138, 139, 140, 141, 142,
    // 143, 144, 145, 146, 147, 148  // UTF8(C3)
    //                Ä    Å    Æ    Ç         É Ñ                  // CHAR
    000, 000, 000, 142, 143, 146, 128, 000, 144, 000, 000, 000, 000, 000, 000,
    000, 165, 000, 000, 000, // ASCII
    // 149, 150, 151, 152, 153, 154, 155, 156, 157, 158, 159, 160, 161, 162,
    // 163, 164, 165, 166, 167, 168
    //      Ö                             Ü              ß    à ä    å    æ è
    000, 153, 000, 000, 000, 000, 000, 154, 000, 000, 225, 133, 000, 000, 000,
    132, 134, 145, 000, 138,
    // 169, 170, 171, 172. 173. 174. 175, 176, 177, 179, 179, 180, 181, 182,
    // 183, 184, 185, 186, 187, 188
    //      ê    ë    ì         î    ï         ñ    ò         ô         ö ù û ü
    000, 136, 137, 141, 000, 140, 139, 000, 164, 149, 000, 147, 000, 148, 000,
    000, 151, 000, 150, 129
  };
#endif
...
//---------------------------------------------------------------------------------------------------------------------
bool
Audio::connecttoSD (const char *path, uint32_t resumeFilePos)
{
  return connecttoFS (SD, path, resumeFilePos);
}
//---------------------------------------------------------------------------------------------------------------------
bool
Audio::connecttoFS (fs::FS &fs, const char *path, uint32_t resumeFilePos)
{
...
//---------------------------------------------------------------------------------------------------------------------
bool
Audio::connecttospeech (const char *speech, const char *lang)
{
...
//---------------------------------------------------------------------------------------------------------------------
bool
Audio::connecttomarytts (const char *speech, const char *lang,
                         const char *voice)
{
...
//---------------------------------------------------------------------------------------------------------------------
void
Audio::urlencode (char *buff, uint16_t buffLen, bool spacesOnly)
{
...
//---------------------------------------------------------------------------------------------------------------------
void
Audio::showID3Tag (const char *tag, const char *value)
{
  ...
  // if(!strcmp(tag, "TAL")) sprintf(m_chbuf, "Album/Movie/Show title: %s",
  // value);
  if (!strcmp (tag, "TBP"))
  ...
  if (!strcmp (tag, "TRC"))
    sprintf (m_chbuf, "ISRC (International Standard Recording Code): %s",
             value);
  if (!strcmp (tag, "TRD"))
...
//---------------------------------------------------------------------------------------------------------------------
void
Audio::unicode2utf8 (char *buff, uint32_t len)
{
  // converts unicode in UTF-8, buff contains the string to be converted up to
  // len range U+1 ... U+FFFF
...
//---------------------------------------------------------------------------------------------------------------------
bool
Audio::latinToUTF8 (char *buff, size_t bufflen)
{
  ...
  // returned utf8 bytelength: >=0xF0 3 bytes, >=0xE0 2 bytes, >=0xC0 1 byte,
  // e.g. e293ab is ⓫
...
//---------------------------------------------------------------------------------------------------------------------
size_t
Audio::readAudioHeader (uint32_t bytes)
{
...
//---------------------------------------------------------------------------------------------------------------------
int
Audio::read_WAV_Header (uint8_t *data, size_t len)
{
  ...
  if (m_controlCounter == 5)
    {
      m_controlCounter++;
      uint16_t fc
          = (uint16_t) (*(data + 0) + (*(data + 1) << 8)); // Format code
      uint16_t nic = (uint16_t) (
          *(data + 2) + (*(data + 3) << 8)); // Number of interleaved channels
      uint32_t sr
...
//---------------------------------------------------------------------------------------------------------------------
int
Audio::read_FLAC_Header (uint8_t *data, size_t len)
{
  static size_t headerSize;
  ...
  if (m_controlCounter == FLAC_SINFO)
    { /* Stream info block */
      ...
      if (m_flacTotalSamplesInStream)
        {
          AUDIO_INFO ("total samples in stream: %u",
      ...
      if (bps != 0 && m_flacTotalSamplesInStream)
        {
          AUDIO_INFO ("audio file duration: %u seconds",
                      m_flacTotalSamplesInStream / m_flacSampleRate);
        }
  ...
  if (m_controlCounter == FLAC_VORBIS)
    { /* VORBIS COMMENT */ // field names
      const char fn[7][12] = { "TITLE",  "VERSION",   "ALBUM", "TRACKNUMBER",
                               "ARTIST", "PERFORMER", "GENRE" };
      int offset;
      size_t l = bigEndian (data, 3);
      ...
      for (int i = 0; i < 7; i++)
        {
          ...
          if (offset >= 0)
            {
              sprintf (m_chbuf, "%s: %s", fn[i],
                       data + offset + strlen (fn[i]) + 1);
...
//---------------------------------------------------------------------------------------------------------------------
int
Audio::read_ID3_Header (uint8_t *data, size_t len)
{
  ...
  if (m_controlCounter == 4)
    { // get the frame size
      ...
      compressed
          = (*(data + 5))
            & 0x80; // Frame is compressed using [#ZLIB zlib] with 4 bytes for
  ...
  if (m_controlCounter == 6)
    {                       // Read the value
      ...
      // 0x80). $01 – UCS-2 encoded Unicode with BOM, in ID3v2.2 and ID3v2.3.
          ...
          if (APIC_seen && audio_id3image)
            {
              ...
              audiofile.seek (pos); // the filepointer could have been changed
                                    // by the user, set it back
            }
          return 0;
        }
    }
...
//---------------------------------------------------------------------------------------------------------------------
int
Audio::read_M4A_Header (uint8_t *data, size_t len)
{
  /*
         ftyp
  ...
  if (m_controlCounter == M4A_FTYP)
    {                                 /* check_m4a_file */
      ...
      if ((m4a != 8) && (isom != 8))
        {
          log_e ("subtype 'MA4 ' or 'isom' expected, but found '%s '",
                 (data + 8));
          stopSong ();
          return -1;
        }
  ...
  if (m_controlCounter == M4A_TRAK)
    { // trak
      if (specialIndexOf (data, "esds", len) > 0)
        {
          int esds = specialIndexOf (
              data, "esds", len); // Packaging/Encapsulation And Setup Data
          uint8_t *pos = data + esds;
          uint8_t len_of_OD = *(
              pos + 12); // length of this OD (which includes the next 2 tags)
          (void)len_of_OD;
          uint8_t len_of_ESD
              = *(pos + 20); // length of this Elementary Stream Descriptor
          (void)len_of_ESD;
          ...
          uint8_t sRate
              = (ASC & 0x0600) >> 7; // next 4 bits Sampling Frequencies
          ...
          if (chConfig > 2)
            {
              log_e ("Channel Configurations with more than 2 channels is not "
                     "allowed!");
            }
  ...
  if (m_controlCounter == M4A_ILST)
    { // ilst
      const char info[12][6]
          = { "nam\0",  "ART\0",  "alb\0", "too\0",  "cmt\0",  "wrt\0",
              "tmpo\0", "trkn\0", "day\0", "cpil\0", "aART\0", "gen\0" };
      int offset;
      for (int i = 0; i < 12; i++)
        {
          offset = specialIndexOf (data, info[i], len,
                                   true); // seek info[] with '\0'
          if (offset > 0)
            {
              offset += 19;
              if (*(data + offset) == 0)
  ...
  if (m_controlCounter == M4A_MDAT)
    { // mdat
      m_audioDataSize
          = bigEndian (data, 4) - 8; // length of this atom - strlen(M4A_MDAT)
      AUDIO_INFO ("Audio-Length: %u", m_audioDataSize);
      retvalue = 8;
      headerSize += 8;
  ...
  if (m_controlCounter == M4A_AMRDY)
    { // almost ready
      m_audioDataStart = headerSize;
      //        m_contentlength = headerSize + m_audioDataSize; // after this
      //        mdat atom there may be other atoms
      if (getDatamode () == AUDIO_LOCALFILE)
        {
          AUDIO_INFO ("Content-Length: %u", m_contentlength);
        }
...
//---------------------------------------------------------------------------------------------------------------------
int
Audio::read_OGG_Header (uint8_t *data, size_t len)
{
  static size_t retvalue = 0;
  ...
  if (m_controlCounter == OGG_FIRST)
    { /* check OGG FIRST PAGES (has no streaming content) */
      ...
      if (specialIndexOf (data + i, "FLAC", 10) == 0)
        ...
        {
          log_e ("ogg/flac support only"); // ogg/vorbis or ogg//opus not
                                           // supported yet
          stopSong ();
          return -1;
        }
      ...
      (void)minor_vers;
      i++;
      uint16_t nonah = bigEndian (
          data + i, 2); // number of non audio headers (0x00 = unknown)
      (void)nonah;
      i += 2;
      if (specialIndexOf (data + i, "fLaC", 10) == 0)
        {
  ...
  if (m_controlCounter == OGG_AMRDY)
    { // ogg almost ready
      ...
      AUDIO_INFO ("FLACDecoder has been initialized, free Heap: %u bytes",
                  ESP.getFreeHeap ());
...
//---------------------------------------------------------------------------------------------------------------------
size_t
Audio::process_m3u8_ID3_Header (uint8_t *packet)
{
  uint8_t ID3version;
  size_t id3Size;
...
Audio::stopSong ()
{
  ...
  if (audiofile)
    {
      // added this before putting 'm_f_localfile = false' in stopSong();
      // shoulf never occur....
      audiofile.close ();
      AUDIO_INFO ("Closing audio file");
...
{ // returns true if all dma_buffs flushed
  ...
  if (getBitsPerSample () > 8)
    ...
    memset (m_outBuff, 128,
            sizeof (m_outBuff)); // Clear OutputBuffer (unsigned, PCM 8u)
          ...
          while (m_validSamples)
            {
              if (!m_f_forceMono)
                ...
                { // mono mode, #100
                  int16_t xy = (m_outBuff[m_curSample * 2]
                                + m_outBuff[m_curSample * 2 + 1])
                               / 2;
                  sample[LEFTCHANNEL] = xy;
                  sample[RIGHTCHANNEL] = xy;
                }
...
Audio::loop ()
{
  ...
  if (m_playlistFormat != FORMAT_M3U8)
    ...
    { // m3u8 datastream only
      ...
      static int32_t remaintime, timestamp1,
          timestamp2; // m3u8 time management
      const char *host;
        ...
        case AUDIO_PLAYLISTDATA:
          ...
          if (host)
          ...
          else
            {
              ...
              setDatamode (AUDIO_DATA); // fake datamode, we have no new
                                        // audiosequence yet, so let audio run
            }
          break;
        case AUDIO_DATA:
          if (m_f_ts)
          ...
          else
            {
              if (m_f_continue)
                { // processWebStream() needs more data
                  remaintime = (int32_t) (m_m3u8_targetDuration * 1000)
                               - (millis () - timestamp1);
  ...
  while (true)
    { // outer while
      ...
      if (m_playlistContent.size () == 100)
        {
          if (m_f_Log)
            log_i ("the maximum number of lines in the playlist has been "
                   "reached");
          break;
        }
      ...
      // 2. no contentLength, but Transfer-Encoding:chunked -> compute
      // chunksize and read until chunksize is reached
              ...
              if (pos > 0)
                {
                  ...
                  int pos1 = indexOf (host, "\"",
                                      0); // http://87.98.217.63:24112/stream
                  if (pos1 > 0)
                    host[pos1]
                        = '\0'; // Now we have an URL for a stream in host.
                }
            }
        }
      ...
      for (int i = 0; i < lines; i++)
        {
          ...
          if (startsWith (m_playlistContent[i], "#EXT-X-STREAM-INF:"))
            {
              if (occurence > 0)
                break; // no more than one #EXT-X-STREAM-INF: (can have
                       // different BANDWIDTH)
              occurence++;
              if ((!endsWith (m_playlistContent[i + 1], "m3u8")
                   && indexOf (m_playlistContent[i + 1], "m3u8?") == -1))
                {
                  // we have a new m3u8 playlist, skip to next line
                  int pos
                      = indexOf (m_playlistContent[i], "CODECS=\"mp4a", 18);
                  // 'mp4a.40.01' AAC Main
                  ...
                  if (pos < 0)
                    { // not found
                      ...
                      if (pos1 < 0)
                        pos1 = 0;
                      log_e ("codec %s in m3u8 playlist not supported",
              ...
              if (!startsWith (m_playlistContent[i], "http"))
                {
                  ...
                  // http://livees.com/prog_index.m3u8 and chunklist022.m3u8
                  // --> http://livees.com/chunklist022.m3u8
                  tmp = (char *)malloc (strlen (m_lastHost)
                                        + strlen (m_playlistContent[i]));
              ...
              return m_playlistContent[i]; // it's a redirection, a new m3u8
                                           // playlist
            }
          ...
          // #EXT-X-DISCONTINUITY
          // #EXTINF:10,title="text=\"Spot Block End\"
          // amgTrackId=\"9876543\"",artist=" ",url="length=\"00:00:00\""
              ...
              if (m_hashQueue.size () == 0)
              ...
              else
                {
                  ...
                  if (!known)
                    {
                      m_hashQueue.insert (m_hashQueue.begin (), hash);
                      m_playlistURL.insert (m_playlistURL.begin (),
                                            strdup (tmp));
                    }
                }
...
Audio::processLocalFile ()
{
  ...
  if (m_audioDataSize)
    {
      availableBytes = min (availableBytes,
                            m_audioDataSize + m_audioDataStart - byteCounter);
    }
          ...
          if (m_resumeFilePos)
            {
              ...
              if (m_avr_bitrate)
                m_audioCurrentTime
                    = ((m_resumeFilePos - m_audioDataStart) / m_avr_bitrate)
                      * 8;
  ...
  if (f_fileDataComplete && InBuff.bufferFilled () < InBuff.getMaxBlockSize ())
    {
      if (InBuff.bufferFilled ())
        {
          if (!readID3V1Tag ())
            {
              int bytesDecoded
                  = sendBytes (InBuff.getReadPtr (), InBuff.bufferFilled ());
              if (bytesDecoded > 2)
                {
      ...
      if (m_f_loop && f_stream)
        { // eof
          AUDIO_INFO ("loop from: %u to: %u", getFilePos (),
                      m_audioDataStart); // TEST loop
          ...
          /*
              ...
              For example: current time   ====progress bar====>  total audio
             duration 3:43        ====================>        3:33
          */
...
Audio::processWebFile ()
{
  ...
  if (m_audioDataSize)
    availableBytes = min (m_audioDataSize - (byteCounter - m_audioDataStart),
                          availableBytes);
  ...
  if (f_webFileDataComplete
      && InBuff.bufferFilled () < InBuff.getMaxBlockSize ())
    {
      ...
      playI2Sremains ();
      stopSong (); // Correct close when play known length sound #74 and before
                   // callback #11
      if (m_f_tts)
        {
  ...
  if (availableBytes)
    {
      ...
      if (m_f_chunked)
        chunkSize = chunkedDataTransfer (&readedBytes);
      int res = _client->read (ts_packet + ts_packetPtr,
                               ts_packetsize - ts_packetPtr);
      if (res > 0)
        {
          ts_packetPtr += res;
          byteCounter += res;
      ...
      if (firstBytes)
        {
          ...
          if (m_controlCounter < 100)
            {
              int res = read_ID3_Header (&ID3Buff[ID3ReadPtr],
...
Audio::playAudioData ()
{
  ...
  int bytesDecoded
      = sendBytes (InBuff.getReadPtr (), InBuff.getMaxBlockSize ());
    ...
    { // outer while
      ...
      while (_client->available ())
        {
          ...
          if (b == '\n')
            {
              if (!pos)
                { // empty line received, is the last line of this
                  // responseHeader
                  if (ct_seen)
                    goto lastToDo;
                  else
                  ...
                  if (pos_slash > 9)
                    {
                      if (!strncmp (c_host, m_lastHost, pos_slash))
                        {
                          AUDIO_INFO ("redirect to new extension at existing "
                                      "host \"%s\"",
                                      c_host);
                          if (m_playlistFormat == FORMAT_M3U8)
                            {
      ...
      // }
      ...
      else if (startsWith (rhl, "icy-br:"))
        {
          ...
          int32_t br = atoi (
              c_bitRate); // Found bitrate tag, read the bitrate in Kbit
          br = br * 1000;
          setBitrate (br);
      ...
      else if (startsWith (rhl, "icy-description:"))
        {
          ...
          while (c_idesc[0] == ' ')
            c_idesc++;
          latinToUTF8 (rhl, sizeof (rhl)); // if already UTF-0 do nothing,
  ...
  switch (m_codec)
    ...
    case CODEC_MP3:
      if (!MP3Decoder_AllocateBuffers ())
        goto exit;
      AUDIO_INFO ("MP3Decoder has been initialized, free Heap: %u bytes",
                  ESP.getFreeHeap ());
      ...
      if (!AACDecoder_IsInit ())
        {
          if (!AACDecoder_AllocateBuffers ())
            goto exit;
          AUDIO_INFO ("AACDecoder has been initialized, free Heap: %u bytes",
                      ESP.getFreeHeap ());
...
//---------------------------------------------------------------------------------------------------------------------
bool
Audio::parseContentType (char *ct)
{
...
//---------------------------------------------------------------------------------------------------------------------
void
Audio::showstreamtitle (const char *ml)
{
  // example for ml:
  ...
  if (m_codec == CODEC_AAC || m_codec == CODEC_M4A)
    {
      ...
      if (answ == 1)
        { // ADTS Header
          ...
          if (AACGetProfile () < 5)
            {
              const char pr[4][23] = { "Main", "LowComplexity",
                                       "Scalable Sampling Rate", "reserved" };
...
//---------------------------------------------------------------------------------------------------------------------
int
Audio::findNextSync (uint8_t *data, size_t len)
{
  ...
  //         -1 the sync word was not found within the block with the length
  //         len
...
//---------------------------------------------------------------------------------------------------------------------
int
Audio::sendBytes (uint8_t *data, size_t len)
{
  int bytesLeft;
...
//---------------------------------------------------------------------------------------------------------------------
void
Audio::compute_audioCurrentTime (int bd)
{
      ...
      if (loop_counter > 20 && loop_counter < 200)
        {
          ...
          if (loop_counter == 199 && m_resumeFilePos)
            {
              m_audioCurrentTime
                  = ((getFilePos () - m_audioDataStart - inBufferFilled ())
                     / m_avr_bitrate)
                    * 8; // #293
            }
        }
...
//---------------------------------------------------------------------------------------------------------------------
bool
Audio::setPinout (uint8_t BCLK, uint8_t LRC, uint8_t DOUT, int8_t DIN,
                  int8_t MCK)
{
...
//---------------------------------------------------------------------------------------------------------------------
bool
Audio::setAudioPlayPosition (uint16_t sec)
{
...
//---------------------------------------------------------------------------------------------------------------------
bool
Audio::audioFileSeek (const float speed)
{
  // 0.5 is half speed
  // 1.0 is normal speed
...
//---------------------------------------------------------------------------------------------------------------------
bool
Audio::setSampleRate (uint32_t sampRate)
{
  if (!sampRate)
...
//---------------------------------------------------------------------------------------------------------------------
bool
Audio::setBitsPerSample (int bits)
{
  if ((bits != 16) && (bits != 8))
    return false;
...
//---------------------------------------------------------------------------------------------------------------------
void
Audio::setI2SCommFMT_LSB (bool commFMT)
{
  // false: I2S communication format is by default I2S_COMM_FORMAT_I2S_MSB,
  // right->left (AC101, PCM5102A) true:  changed to I2S_COMM_FORMAT_I2S_LSB
  // for some DACs (PT8211)
...
#if ESP_ARDUINO_VERSION_MAJOR >= 2
      m_i2s_config.communication_format
          = (i2s_comm_format_t) (I2S_COMM_FORMAT_STAND_MSB); // v >= 2.0.0
#else
      m_i2s_config.communication_format = (i2s_comm_format_t) (
...
//---------------------------------------------------------------------------------------------------------------------
bool
Audio::playSample (int16_t sample[2])
{
  ...
  sample[LEFTCHANNEL] = sample[LEFTCHANNEL]
                        >> 1; // half Vin so we can boost up to 6dB in filters
  ...
  esp_err_t err = i2s_write ((i2s_port_t)m_i2s_num, (const char *)&s32,
                             sizeof (uint32_t), &m_i2s_bytesWritten, 100);
  if (err != ESP_OK)
    {
  ...
  if (m_i2s_bytesWritten < 4)
    {
      log_e ("Can't stuff any more in I2S..."); // increase waitingtime or
                                                // outputbuffer
      return false;
    }
  return true;
...
//---------------------------------------------------------------------------------------------------------------------
void
Audio::setTone (int8_t gainLowPass, int8_t gainBandPass, int8_t gainHighPass)
{
...
//---------------------------------------------------------------------------------------------------------------------
void
Audio::IIR_calculateCoefficients (int8_t G0, int8_t G1, int8_t G2)
...
//---------------------------------------------------------------------------------------------------------------------
int16_t *
Audio::IIR_filterChain0 (int16_t iir_in[2], bool clear)
{ // Infinite Impulse Response (IIR) filters
  ...
  outSample[LEFTCHANNEL]
      = m_filter[0].a0 * inSample[LEFTCHANNEL]
  ...
  m_filterBuff[0][z2][out][LEFTCHANNEL]
  ...
  outSample[RIGHTCHANNEL]
      = m_filter[0].a0 * inSample[RIGHTCHANNEL]
  ...
  m_filterBuff[0][z2][in][RIGHTCHANNEL]
      = m_filterBuff[0][z1][in][RIGHTCHANNEL];
  m_filterBuff[0][z1][in][RIGHTCHANNEL] = inSample[RIGHTCHANNEL];
  m_filterBuff[0][z2][out][RIGHTCHANNEL]
      = m_filterBuff[0][z1][out][RIGHTCHANNEL];
...
//---------------------------------------------------------------------------------------------------------------------
int16_t *
Audio::IIR_filterChain1 (int16_t iir_in[2], bool clear)
{ // Infinite Impulse Response (IIR) filters
  ...
  outSample[LEFTCHANNEL]
      = m_filter[1].a0 * inSample[LEFTCHANNEL]
  ...
  m_filterBuff[1][z2][in][RIGHTCHANNEL]
      = m_filterBuff[1][z1][in][RIGHTCHANNEL];
  m_filterBuff[1][z1][in][RIGHTCHANNEL] = inSample[RIGHTCHANNEL];
  m_filterBuff[1][z2][out][RIGHTCHANNEL]
      = m_filterBuff[1][z1][out][RIGHTCHANNEL];
...
//---------------------------------------------------------------------------------------------------------------------
int16_t *
Audio::IIR_filterChain2 (int16_t iir_in[2], bool clear)
{ // Infinite Impulse Response (IIR) filters
  ...
  outSample[LEFTCHANNEL]
      = m_filter[2].a0 * inSample[LEFTCHANNEL]
  ...
  m_filterBuff[2][z2][in][RIGHTCHANNEL]
      = m_filterBuff[2][z1][in][RIGHTCHANNEL];
  m_filterBuff[2][z1][in][RIGHTCHANNEL] = inSample[RIGHTCHANNEL];
  m_filterBuff[2][z2][out][RIGHTCHANNEL]
      = m_filterBuff[2][z1][out][RIGHTCHANNEL];
...
//----------------------------------------------------------------------------------------------------------------------
bool
Audio::ts_parsePacket (uint8_t *packet, uint8_t *packetStart,
  ...
  if (PID == 0)
    {
      ...
      for (int i = startOfProgramNums; i <= sectionLength;
           i += lengthOfPATValue)
        {
          ...
          program_map_PID = ((packet[PLS + i + 2] & 0x1F) << 8)
                            | (packet[PLS + i + 3] & 0xFF);
          if (m_f_Log)
            log_i ("Program Num: 0x%04X(%d) PMT PID: 0x%04X(%d)",
                   program_number, program_number, program_map_PID,
                   program_map_PID);
      ...
      else
        {
          ...
          if (firstByte == 0x00 && secondByte == 0x00 && thirdByte == 0x01)
            { // Packet start code prefix
              ...
              uint8_t PES_HeaderDataLength
                  = packet[posOfPacketStart + 8] & 0xFF;
              if (m_f_Log)
                log_i ("PES_headerDataLength %d", PES_HeaderDataLength);
              int PES_PacketLength
                  = ((packet[posOfPacketStart + 4] & 0xFF) << 8)
          ...
          if (PID == pidsOfPMT.pids[i])
            {
              ...
              if (m_f_Log)
                log_i ("Section Length: %d", sectionLength);
              int programInfoLength = ((packet[PLS + 10] & 0x0F) << 8)
                                      | (packet[PLS + 11] & 0xFF);
              if (m_f_Log)
              ...
              while (cursor < sectionLength - 1)
                {
                  ...
                  if (m_f_Log)
                    log_i ("Stream Type: 0x%02X Elementary PID: 0x%04X",
...
//----------------------------------------------------------------------------------------------------------------------
uint16_t
Audio::readMetadata (uint16_t maxBytes, bool first)
{
  ...
  if (!metalen)
    {
      ...
      metalen
          = b * 16; // New count for metadata including length byte, max 4096
      pos_ml = 0;
  ...
  if (pos_ml == metalen)
    {
      ...
      if (strlen (m_chbuf))
        { // Any info present?
          ...
          // Isolate the StreamTitle, remove leading and trailing quotes if
          // present.
          latinToUTF8 (m_chbuf, m_chbufSize); // convert to UTF-8 if necessary
          int pos = indexOf (m_chbuf, "song_spot",
                             0); // remove some irrelevant infos
          if (pos > 3)
            { // e.g. song_spot="T" MediaBaseId="0" itunesTrackId="0"
              m_chbuf[pos] = 0;
            }
          showstreamtitle (
              m_chbuf); // Show artist and title if present in metadata
        }
      m_metacount = m_metaint;
      metalen = 0;
      pos_ml = 0;
...
//----------------------------------------------------------------------------------------------------------------------
size_t
Audio::chunkedDataTransfer (uint8_t *bytes)
{
...
Audio::readID3V1Tag ()
{
  ...
  if (InBuff.bufferFilled () == 128
      && startsWith ((const char *)InBuff.getReadPtr (), "TAG"))
    { // maybe a V1.x TAG
      char title[31];
...
//----------------------------------------------------------------------------------------------------------------------
void
Audio::slowStreamDetection (uint32_t inBuffFilled, uint32_t maxFrameSize)
{
...
//----------------------------------------------------------------------------------------------------------------------
void
Audio::lostStreamDetection (uint32_t bytesAvail)
{
