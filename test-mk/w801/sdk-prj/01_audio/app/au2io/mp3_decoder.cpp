...
const int ISFMpeg1[2][7] PROGMEM
    = { { 0x00000000, 0x0d8658ba, 0x176cf5d0, 0x20000000, 0x28930a2f,
          0x3279a745, 0x40000000 },
        { 0x00000000, 0x13207f5c, 0x2120fb83, 0x2d413ccc, 0x39617e16,
          0x4761fa3d, 0x5a827999 } };
...
void
SetBitstreamPointer (BitStreamInfo_t *bsi, int nBytes, unsigned char *buf)
{
  /* init bitstream */
  bsi->bytePtr = buf;
...
//----------------------------------------------------------------------------------------------------------------------
void
RefillBitstreamCache (BitStreamInfo_t *bsi)
{
...
//----------------------------------------------------------------------------------------------------------------------
unsigned int
GetBits (BitStreamInfo_t *bsi, int nBits)
{
...
//----------------------------------------------------------------------------------------------------------------------
int
CalcBitsUsed (BitStreamInfo_t *bsi, unsigned char *startBuf, int startOffset)
{
  int bitsUsed;
...
//----------------------------------------------------------------------------------------------------------------------
int
UnpackFrameHeader (unsigned char *buf)
{
  int verIdx;
  ...
  m_sMode = (StereoMode_t) (
      (buf[3] >> 6) & 0x03); /* maps to correct enum (see definition) */
  ...
  if (m_FrameHeader->brIdx)
    {
      m_MP3DecInfo->bitrate
          = ((int)bitrateTab[m_MPEGVersion][m_FrameHeader->layer - 1]
                            [m_FrameHeader->brIdx])
            * 1000;
      /* nSlots = total frame bytes (from table) - sideInfo bytes - header -
       * CRC (if present) + pad (if present) */
      m_MP3DecInfo->nSlots
          = (int)slotTab[m_MPEGVersion][m_FrameHeader->srIdx]
                        [m_FrameHeader->brIdx]
            - (int)sideBytesTab[m_MPEGVersion][(m_sMode == Mono ? 0 : 1)] - 4
            - (m_FrameHeader->crc ? 2 : 0)
            + (m_FrameHeader->paddingBit ? 1 : 0);
    }
...
//----------------------------------------------------------------------------------------------------------------------
int
UnpackSideInfo (unsigned char *buf)
{
  int gr, ch, bd, nBytes;
          ...
          if (sis->winSwitchFlag)
            {
              ...
              sis->blockType = GetBits (
                  bsi, 2); /* 0 = normal, 1 = start, 2 = short, 3 = stop */
              sis->mixedBlock
                  = GetBits (bsi, 1); /* 0 = not mixed, 1 = mixed */
 ...
 *              ScaleFactorInfoSub from granule 0 (for granule 1, if scfsi[i]
 *is set, then we just replicate the scale factors from granule 0 in the i'th
 *set of scalefactor bands)
 *
 ...
 **********************************************************************************************************************/
void
UnpackSFMPEG1 (BitStreamInfo_t *bsi, SideInfoSub_t *sis,
 ...
 **********************************************************************************************************************/
void
UnpackSFMPEG2 (BitStreamInfo_t *bsi, SideInfoSub_t *sis,
 ...
 **********************************************************************************************************************/
int
UnpackScaleFactors (unsigned char *buf, int *bitOffset, int bitsAvail, int gr,
                    int ch)
{
  int bitsUsed;
 ...
 **********************************************************************************************************************/
int
MP3FindSyncWord (unsigned char *buf, int nBytes)
{
  int i;
 ...
 *not actually the next frame header) this could be made more error-resilient
 *by checking several frames in a row and verifying that nSlots is the same in
 *each case since free mode requires CBR (see spec) we generally only call this
 *function once (first frame) then store the result (nSlots) and just use it
 *from then on
 **********************************************************************************************************************/
int
MP3FindFreeSync (unsigned char *buf, unsigned char firstFH[4], int nBytes)
{
  int offset = 0;
 ...
 *
 * Inputs:        pointer to buffer containing valid MP3 frame header (located
 ...
 **********************************************************************************************************************/
int
MP3GetNextFrameInfo (unsigned char *buf)
{
 ...
 **********************************************************************************************************************/
void
MP3ClearBadFrame (short *outbuf)
{
  int i;
  for (i = 0; i < m_MP3DecInfo->nGrans * m_MP3DecInfo->nGranSamps
                      * m_MP3DecInfo->nChans;
       i++)
    outbuf[i] = 0;
}
 ...
 *samples flag indicating whether MP3 data is normal MPEG format (useSize = 0)
 ...
 **********************************************************************************************************************/
int
MP3Decode (unsigned char *inbuf, int *bytesLeft, short *outbuf, int useSize)
{
      ...
      if (!m_MP3DecInfo->freeBitrateFlag)
        {
          ...
          m_MP3DecInfo->bitrate
              = (freeFrameBytes * m_MP3DecInfo->samprate * 8)
                / (m_MP3DecInfo->nGrans * m_MP3DecInfo->nGranSamps);
        }
      m_MP3DecInfo->nSlots = m_MP3DecInfo->freeBitrateSlots
  ...
  if (useSize)
    {
      ...
      if (m_MP3DecInfo->mainDataBegin != 0 || m_MP3DecInfo->nSlots <= 0)
        {
          /* error - non self-contained frame, or missing frame (size <= 0),
           * could do loss concealment here */
          MP3ClearBadFrame (outbuf);
          return ERR_MP3_INVALID_FRAMEHEADER;
        }
      ...
      if (m_MP3DecInfo->mainDataBytes >= m_MP3DecInfo->mainDataBegin)
        {
          ...
          memmove (m_MP3DecInfo->mainBuf,
                   m_MP3DecInfo->mainBuf + m_MP3DecInfo->mainDataBytes
                       - m_MP3DecInfo->mainDataBegin,
                   m_MP3DecInfo->mainDataBegin);
          memcpy (m_MP3DecInfo->mainBuf + m_MP3DecInfo->mainDataBegin, inbuf,
                  m_MP3DecInfo->nSlots);

          m_MP3DecInfo->mainDataBytes
              = m_MP3DecInfo->mainDataBegin + m_MP3DecInfo->nSlots;
          inbuf += m_MP3DecInfo->nSlots;
          *bytesLeft -= (m_MP3DecInfo->nSlots);
      ...
      else
        {
          ...
          memcpy (m_MP3DecInfo->mainBuf + m_MP3DecInfo->mainDataBytes, inbuf,
                  m_MP3DecInfo->nSlots);
          m_MP3DecInfo->mainDataBytes += m_MP3DecInfo->nSlots;
          inbuf += m_MP3DecInfo->nSlots;
          *bytesLeft -= (m_MP3DecInfo->nSlots);
  ...
  for (gr = 0; gr < m_MP3DecInfo->nGrans; gr++)
    {
      ...
      if (Subband (outbuf
                   + gr * m_MP3DecInfo->nGranSamps * m_MP3DecInfo->nChans)
          < 0)
        {
          MP3ClearBadFrame (outbuf);
 ...
 *all the internal buffers needed for decoding)
 *
...
bool
MP3Decoder_AllocateBuffers (void)
{
  if (!m_MP3DecInfo)
    {
      m_MP3DecInfo
          = (MP3DecInfo_t *)__malloc_heap_psram (sizeof (MP3DecInfo_t));
    }
  if (!m_FrameHeader)
    {
      m_FrameHeader
          = (FrameHeader_t *)__malloc_heap_psram (sizeof (FrameHeader_t));
    }
  if (!m_SideInfo)
    {
  ...
  if (!m_ScaleFactorJS)
    {
      m_ScaleFactorJS
          = (ScaleFactorJS_t *)__malloc_heap_psram (sizeof (ScaleFactorJS_t));
    }
  if (!m_HuffmanInfo)
    {
      m_HuffmanInfo
          = (HuffmanInfo_t *)__malloc_heap_psram (sizeof (HuffmanInfo_t));
    }
  if (!m_DequantInfo)
    {
      m_DequantInfo
          = (DequantInfo_t *)__malloc_heap_psram (sizeof (DequantInfo_t));
    }
  if (!m_IMDCTInfo)
    {
  ...
  if (!m_SubbandInfo)
    {
      m_SubbandInfo
          = (SubbandInfo_t *)__malloc_heap_psram (sizeof (SubbandInfo_t));
    }
  if (!m_MP3FrameInfo)
    {
      m_MP3FrameInfo
          = (MP3FrameInfo_t *)__malloc_heap_psram (sizeof (MP3FrameInfo_t));
    }

  if (!m_MP3DecInfo || !m_FrameHeader || !m_SideInfo || !m_ScaleFactorJS
...
int
DecodeHuffmanPairs (int *xy, int nVals, int tabIdx, int bitsLeft,
                    unsigned char *buf, int bitOffset)
{
  ...
  if (tabType == noBits)
  ...
  else if (tabType == oneShot)
    {
      ...
      maxBits = (int)((((unsigned short)(pgm_read_word (&tBase[0])) >> 0)
                       & 0x000f));
      tBase++;
      padBits = 0;
      while (nVals > 0)
        {
          ...
          /* largest maxBits = 9, plus 2 for sign bits, so make sure cache has
           * at least 11 bits */
      ...
      while (nVals > 0)
        {
          ...
          while (nVals > 0 && cachedBits >= 11)
            {
              maxBits
                  = (int)((((unsigned short)(pgm_read_word (&tCurr[0]))) >> 0)
                          & 0x000f);
...
// no improvement with section=data
int
DecodeHuffmanQuads (int *vwxy, int nVals, int tabIdx, int bitsLeft,
                    unsigned char *buf, int bitOffset)
{
  int i, v, w, x, y;
 ...
 *starting bit in buf[0] number of bits in the Huffman data section of the
 *frame (could include padding bits) index of current granule and channel
 *
...
int
DecodeHuffman (unsigned char *buf, int *bitOffset, int huffBlockBits, int gr,
               int ch)
{
  ...
  rEnd[3] = (m_MAX_NSAMP < (2 * sis->nBigvals) ? m_MAX_NSAMP
                                               : (2 * sis->nBigvals));
  ...
  for (i = 0; i < 3; i++)
    {
      bitsUsed = DecodeHuffmanPairs (
          m_HuffmanInfo->huffDecBuf[ch] + rEnd[i], rEnd[i + 1] - rEnd[i],
  ...
  if (m_FrameHeader->modeExt & 0x01)
    {
      ...
      if (m_MPEGVersion == MPEG1)
        {
          IntensityProcMPEG1 (
              m_HuffmanInfo->huffDecBuf, nSamps, &m_ScaleFactorInfoSub[gr][1],
              &m_CriticalBandInfo[0], m_FrameHeader->modeExt >> 1,
              m_SideInfoSub[gr][1].mixedBlock, mOut);
        }
      else
        {
          IntensityProcMPEG2 (m_HuffmanInfo->huffDecBuf, nSamps,
                              &m_ScaleFactorInfoSub[gr][1],
                              &m_CriticalBandInfo[0], m_ScaleFactorJS,
 ...
 **********************************************************************************************************************/
int
DequantBlock (int *inbuf, int *outbuf, int num, int scale)
{
  int tab4[4];
  ...
  // scalei = MIN(scale >> 2, 31);   /* smallest input scale = -47, so smallest
  // scalei = -12 */
 ...
 *codewords
 *
 * Inputs:      sample buffer (decoded Huffman codewords), length = m_MAX_NSAMP
 *samples work buffer for reordering short-block, length = m_MAX_REORDER_SAMPS
 ...
 **********************************************************************************************************************/
int
DequantChannel (int *sampleBuf, int *workBuf, int *nonZeroBound,
  ...
  for (cb = 0; cb < cbEndL; cb++)
    {
      ...
      gainI = 210 - globalGain
              + sfactMultiplier
                    * (sfis->l[cb] + (sis->preFlag ? (int)preTab[cb] : 0));
 ...
 **********************************************************************************************************************/
void
MidSideProc (int x[m_MAX_NCHAN][m_MAX_NSAMP], int nSamps, int mOut[2])
{
 ...
 **********************************************************************************************************************/
void
IntensityProcMPEG1 (int x[m_MAX_NCHAN][m_MAX_NSAMP], int nSamps,
 ...
 **********************************************************************************************************************/
void
IntensityProcMPEG2 (int x[m_MAX_NCHAN][m_MAX_NSAMP], int nSamps,
                    ScaleFactorInfoSub_t *sfis, CriticalBandInfo_t *cbi,
 ...
 *18, since nZB is the first ZERO sample above which all other samples are also
 *zero max gain per sample = 1.372 MAX(i) (abs(csa[i][0]) + abs(csa[i][1]))
 *              bits gained = 0
...
void
WinPrevious (int *xPrev, int *xPrevWin, int btPrev)
{
...
int
FreqInvertRescale (int *y, int *xPrev, int blockIdx, int es)
{
 ...
 *type (0,1,2,3) of current and previous block current block index (for
 *deciding whether to do frequency inversion) number of guard bits in input
 *vector
 *
      ...
      for (i = 0; i < 9; i++)
        {
          ...
          xo = MULSHIFT32 (
              c, xo); /* 2*c18*xOdd (mul by 2 implicit in scaling)  */
          xe >>= 2;

          s = -(
              *xPrev); /* sum from last block (always at least 2 guard bits) */
          d = -(xe - xo); /* gain 2 int bits, don't shift xo (effective << 1 to
                             eat sign bit, << 1 for mul by 2) */
          (*xPrev++)
              = xe
                + xo; /* symmetry - xPrev[i] = xPrev[17-i] for long blocks */
          t = s - d;
...
// barely faster in RAM
int
IMDCT12x3 (int *xCurr, int *xPrev, int *y, int btPrev, int blockIdx, int gb)
{
 ...
 *granule/channel BlockCount struct with necessary info number of non-zero
 *input and overlap blocks number of long blocks in input vector (rest assumed
 *to be short blocks) number of blocks which use long window (type) 0 in case
 *of mixed block (bc->currWinSwitch, 0 for non-mixed blocks)
 *
 ...
 **********************************************************************************************************************/
int
HybridTransform (int *xCurr, int *xPrev, int y[m_BLOCK_SIZE][m_NBANDS],
      ...
      for (j = 0; j < 9; j++)
        {
          ...
          /* frequency inversion on odd blocks/odd samples (flip sign if i odd,
           * j odd) */
          xp = xPrevWin[2 * j + 1] << 2;
...
IMDCT (int gr, int ch)
{
  ...
  m_IMDCTInfo->numPrevIMDCT[ch] = HybridTransform (
      m_HuffmanInfo->huffDecBuf[ch], m_IMDCTInfo->overBuf[ch],
...
static const uint8_t FDCT32s1s2[16]
    = { 5, 3, 3, 2, 2, 1, 1, 1, 1, 1, 1, 1, 1, 2, 2, 4 };
 ...
 **********************************************************************************************************************/
inline short
ClipToShort (int x, int fracBits)
{
 ...
 **********************************************************************************************************************/
void
PolyphaseMono (short *pcm, int *vbuf, const uint32_t *coefBase)
{
  int i;
  const uint32_t *coef;
  int *vb1;
  ...
  rndVal = (uint64_t) (
      1ULL << ((m_DQ_FRACBITS_OUT - 2 - 2 - 15) - 1 + (32 - m_CSHIFT)));
  ...
  *(pcm + 0) = ClipToShort ((int)SAR64 (sum1L, (32 - m_CSHIFT)),
  ...
  *(pcm + 16) = ClipToShort ((int)SAR64 (sum1L, (32 - m_CSHIFT)),
                             m_DQ_FRACBITS_OUT - 2 - 2 - 15);
  ...
  for (i = 15; i > 0; i--)
    {
      ...
      vb1 += 64;
      *(pcm) = ClipToShort ((int)SAR64 (sum1L, (32 - m_CSHIFT)),
                            m_DQ_FRACBITS_OUT - 2 - 2 - 15);
      *(pcm + 2 * i) = ClipToShort ((int)SAR64 (sum2L, (32 - m_CSHIFT)),
                                    m_DQ_FRACBITS_OUT - 2 - 2 - 15);
      pcm++;
    }
}
 ...
 **********************************************************************************************************************/
void
PolyphaseStereo (short *pcm, int *vbuf, const uint32_t *coefBase)
{
  int i;
  const uint32_t *coef;
  int *vb1;
  ...
  rndVal = (uint64_t) (
      1 << ((m_DQ_FRACBITS_OUT - 2 - 2 - 15) - 1 + (32 - m_CSHIFT)));
  ...
  *(pcm + 0) = ClipToShort ((int)SAR64 (sum1L, (32 - m_CSHIFT)),
                            m_DQ_FRACBITS_OUT - 2 - 2 - 15);
  *(pcm + 1) = ClipToShort ((int)SAR64 (sum1R, (32 - m_CSHIFT)),
                            m_DQ_FRACBITS_OUT - 2 - 2 - 15);
  ...
  *(pcm + 2 * 16 + 0) = ClipToShort ((int)SAR64 (sum1L, (32 - m_CSHIFT)),
                                     m_DQ_FRACBITS_OUT - 2 - 2 - 15);
  *(pcm + 2 * 16 + 1) = ClipToShort ((int)SAR64 (sum1R, (32 - m_CSHIFT)),
                                     m_DQ_FRACBITS_OUT - 2 - 2 - 15);
  ...
  for (i = 15; i > 0; i--)
    {
      ...
      *(pcm + 1) = ClipToShort ((int)SAR64 (sum1R, (32 - m_CSHIFT)),
                                m_DQ_FRACBITS_OUT - 2 - 2 - 15);
      *(pcm + 2 * 2 * i + 0) = ClipToShort (
          (int)SAR64 (sum2L, (32 - m_CSHIFT)), m_DQ_FRACBITS_OUT - 2 - 2 - 15);
      *(pcm + 2 * 2 * i + 1) = ClipToShort (
          (int)SAR64 (sum2R, (32 - m_CSHIFT)), m_DQ_FRACBITS_OUT - 2 - 2 - 15);
      pcm += 2;
    }
}
