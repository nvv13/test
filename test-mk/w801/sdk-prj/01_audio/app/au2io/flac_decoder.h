...
typedef struct FLACMetadataBlock_t
{
  ...
  uint32_t
      maxframesize; // May be 0 to imply the value is not known.
                    //----------------------------------------------------------------------------------------
                    // Sample rate in Hz. Though 20 bits are available,
                    // the maximum sample rate is limited by the structure of
                    // frame headers to 655350Hz.
  uint32_t
      sampleRate; // Also, a value of 0 is invalid.
                  //----------------------------------------------------------------------------------------
                  // Number of channels FLAC supports from 1 to 8 channels
  uint8_t
      numChannels; // 000 : 1 channel .... 111 : 8 channels
                   //----------------------------------------------------------------------------------------
                   // Sample size in bits:
  ...
  uint64_t
      totalSamples; // of channels. A value of zero here means the number of
...
typedef struct FLACFrameHeader_t
{
  ...
  uint8_t
      blockingStrategy; // 1 : variable-blocksize stream; frame header encodes
                        // the sample number
                        //----------------------------------------------------------------------------------------
                        ...
                        // 576/1152/2304/4608 0110 : get 8 bit (blocksize-1)
                        // from end of header 0111 : get 16 bit (blocksize-1)
                        // from end of header
  uint8_t
      blockSizeCode; // 1000-1111 : 256 * (2^(n-8)) samples, i.e.
                     ...
                     // header 1101 : get 16 bit sample rate (in Hz) from end
                     // of header 1110 : get 16 bit sample rate (in tens of Hz)
                     // from end of header
  uint8_t
  ...
  uint8_t
      chanAsgn; // 1011-1111 : reserved
                ...
                // 110 : 24 bits per sample
  uint8_t
      sampleSizeCode; // 111 : reserved
                      //----------------------------------------------------------------------------------------
  uint32_t
      totalSamples; // totalSamplesInStream
                    //----------------------------------------------------------------------------------------
...
int8_t decodeFixedPredictionSubframe (uint8_t predOrder, uint8_t sampleDepth,
                                      uint8_t ch);
int8_t decodeLinearPredictiveCodingSubframe (int lpcOrder, int sampleDepth,
                                             uint8_t ch);
