...
//----------------------------------------------------------------------------------------------------------------------
bool
FLACDecoder_AllocateBuffers (void)
{
  if (psramFound ())
    {
      ...
      if (!FLACFrameHeader)
        {
          FLACFrameHeader
              = (FLACFrameHeader_t *)ps_malloc (sizeof (FLACFrameHeader_t));
        }
      if (!FLACMetadataBlock)
        {
          FLACMetadataBlock = (FLACMetadataBlock_t *)ps_malloc (
              sizeof (FLACMetadataBlock_t));
        }
      if (!FLACsubFramesBuff)
        {
          FLACsubFramesBuff = (FLACsubFramesBuff_t *)ps_malloc (
              sizeof (FLACsubFramesBuff_t));
        }
    }
  else
    {
      if (!FLACFrameHeader)
        {
          FLACFrameHeader
              = (FLACFrameHeader_t *)malloc (sizeof (FLACFrameHeader_t));
        }
      if (!FLACMetadataBlock)
        {
          FLACMetadataBlock
              = (FLACMetadataBlock_t *)malloc (sizeof (FLACMetadataBlock_t));
        }
      if (!FLACsubFramesBuff)
        {
          FLACsubFramesBuff
              = (FLACsubFramesBuff_t *)malloc (sizeof (FLACsubFramesBuff_t));
        }
    }
...
FLACDecoder_FreeBuffers ()
{
  if (FLACFrameHeader)
    {
      free (FLACFrameHeader);
      FLACFrameHeader = NULL;
    }
  if (FLACMetadataBlock)
    {
      free (FLACMetadataBlock);
      FLACMetadataBlock = NULL;
    }
  if (FLACsubFramesBuff)
    {
      free (FLACsubFramesBuff);
      FLACsubFramesBuff = NULL;
    }
}
//----------------------------------------------------------------------------------------------------------------------
//            B I T R E A D E R
...
int64_t
readRiceSignedInt (uint8_t param)
{
  long val = 0;
  while (readUint (1) == 0)
    val++;
...
//----------------------------------------------------------------------------------------------------------------------
void
FLACSetRawBlockParams (uint8_t Chans, uint32_t SampRate, uint8_t BPS,
...
//----------------------------------------------------------------------------------------------------------------------
int
FLACFindSyncWord (unsigned char *buf, int nBytes)
{
  int i;
...
//----------------------------------------------------------------------------------------------------------------------
int
FLACFindOggSyncWord (unsigned char *buf, int nBytes)
{
  int i;
  ...
  for (i = 0; i < nBytes - 1; i++)
    {
      if ((buf[i + 0] == 'O') && (buf[i + 1] == 'g') && (buf[i + 2] == 'g')
          && (buf[i + 3] == 'S'))
        {
          FLACDecoderReset ();
          log_i ("OggS found");
...
//----------------------------------------------------------------------------------------------------------------------
int
FLACparseOggHeader (unsigned char *buf)
{
  uint8_t i = 0;
...
//----------------------------------------------------------------------------------------------------------------------
int8_t
FLACDecode (uint8_t *inbuf, int *bytesLeft, short *outbuf)
{
  ...
  if (m_status == DECODE_FRAME)
    { // Read a ton of header fields, and ignore most of them

      if ((inbuf[0] == 'O') && (inbuf[1] == 'g') && (inbuf[2] == 'g')
          && (inbuf[3] == 'S'))
        {
          *bytesLeft -= 4;
          m_f_OggS_found = true;
      ...
      if (!FLACMetadataBlock->numChannels)
        {
          if (FLACFrameHeader->chanAsgn == 0)
            FLACMetadataBlock->numChannels = 1;
          if (FLACFrameHeader->chanAsgn == 1)
            FLACMetadataBlock->numChannels = 2;
      ...
      if (!FLACMetadataBlock->bitsPerSample)
        {
          if (FLACFrameHeader->sampleSizeCode == 1)
            FLACMetadataBlock->bitsPerSample = 8;
          if (FLACFrameHeader->sampleSizeCode == 2)
            FLACMetadataBlock->bitsPerSample = 12;
          if (FLACFrameHeader->sampleSizeCode == 4)
            FLACMetadataBlock->bitsPerSample = 16;
          if (FLACFrameHeader->sampleSizeCode == 5)
            FLACMetadataBlock->bitsPerSample = 20;
          if (FLACFrameHeader->sampleSizeCode == 6)
            FLACMetadataBlock->bitsPerSample = 24;
        }
      ...
      if (!FLACMetadataBlock->sampleRate)
        {
          if (FLACFrameHeader->sampleRateCode == 1)
            FLACMetadataBlock->sampleRate = 88200;
          if (FLACFrameHeader->sampleRateCode == 2)
            FLACMetadataBlock->sampleRate = 176400;
          if (FLACFrameHeader->sampleRateCode == 3)
            FLACMetadataBlock->sampleRate = 192000;
          if (FLACFrameHeader->sampleRateCode == 4)
            FLACMetadataBlock->sampleRate = 8000;
          if (FLACFrameHeader->sampleRateCode == 5)
            FLACMetadataBlock->sampleRate = 16000;
          if (FLACFrameHeader->sampleRateCode == 6)
            FLACMetadataBlock->sampleRate = 22050;
          if (FLACFrameHeader->sampleRateCode == 7)
            FLACMetadataBlock->sampleRate = 24000;
          if (FLACFrameHeader->sampleRateCode == 8)
            FLACMetadataBlock->sampleRate = 32000;
          if (FLACFrameHeader->sampleRateCode == 9)
            FLACMetadataBlock->sampleRate = 44100;
          if (FLACFrameHeader->sampleRateCode == 10)
            FLACMetadataBlock->sampleRate = 48000;
          if (FLACFrameHeader->sampleRateCode == 11)
            FLACMetadataBlock->sampleRate = 96000;
        }
      ...
      if (FLACFrameHeader->blockSizeCode == 1)
        m_blockSize = 192;
      else if (2 <= FLACFrameHeader->blockSizeCode
               && FLACFrameHeader->blockSizeCode <= 5)
      ...
      else if (8 <= FLACFrameHeader->blockSizeCode
               && FLACFrameHeader->blockSizeCode <= 15)
      ...
      if (FLACFrameHeader->sampleRateCode == 12)
        readUint (8);
      else if (FLACFrameHeader->sampleRateCode == 13
               || FLACFrameHeader->sampleRateCode == 14)
        {
          readUint (16);
        }
      readUint (8);
  ...
  if (m_status == OUT_SAMPLES)
    { // Write the decoded samples
      ...
      // once therefore we need often more than one loop (split outputblock
      // into pieces)
      uint16_t blockSize;
      static uint16_t offset = 0;
      if (m_blockSize < outBuffSize + offset)
        blockSize = m_blockSize - offset;
      else
        blockSize = outBuffSize;
      ...
      if (offset != m_blockSize)
        return GIVE_NEXT_LOOP;
      offset = 0;
      if (offset > m_blockSize)
        {
          log_e ("offset has a wrong value");
        }
    }
  ...
  //    FLACMetadataBlock->numChannels * (16/8); log_i("m_compressionRatio %
  //    f", m_compressionRatio);
  *bytesLeft = m_bytesAvail;
  m_status = DECODE_FRAME;
  return ERR_FLAC_NONE;
}
...
FLACGetBitRate ()
{
  if (FLACMetadataBlock->totalSamples)
    {
      float BitsPerSamp = (float)FLACMetadataBlock->audioDataLength
                          / (float)FLACMetadataBlock->totalSamples * 8;
  ...
  if (FLACGetSampRate ())
    {
      uint32_t afd = FLACGetTotoalSamplesInStream ()
                     / FLACGetSampRate (); // AudioFileDuration
      return afd;
    }
  return 0;
}
...
decodeSubframes ()
{
  if (FLACFrameHeader->chanAsgn <= 7)
    ...
    {
      decodeSubframe (FLACMetadataBlock->bitsPerSample
                          + (FLACFrameHeader->chanAsgn == 9 ? 1 : 0),
                      0);
      decodeSubframe (FLACMetadataBlock->bitsPerSample
                          + (FLACFrameHeader->chanAsgn == 9 ? 0 : 1),
                      1);
          ...
          for (int i = 0; i < m_blockSize; i++)
            {
              ...
              long right
                  = FLACsubFramesBuff->samplesBuffer[0][i] - (side >> 1);
...
//----------------------------------------------------------------------------------------------------------------------
int8_t
decodeSubframe (uint8_t sampleDepth, uint8_t ch)
{
  int8_t ret = 0;
  readUint (1);
...
//----------------------------------------------------------------------------------------------------------------------
int8_t
decodeFixedPredictionSubframe (uint8_t predOrder, uint8_t sampleDepth,
                               uint8_t ch)
{
  uint8_t ret = 0;
  ...
  if (predOrder == 0)
    coefs.resize (0);
  if (predOrder == 1)
    coefs.push_back (1); // FIXED_PREDICTION_COEFFICIENTS
  if (predOrder == 2)
    {
      coefs.push_back (2);
      coefs.push_back (-1);
  ...
  if (predOrder > 4)
    return ERR_FLAC_PREORDER_TOO_BIG; // Error: preorder > 4"
  restoreLinearPrediction (ch, 0);
  return ERR_FLAC_NONE;
}
//----------------------------------------------------------------------------------------------------------------------
int8_t
decodeLinearPredictiveCodingSubframe (int lpcOrder, int sampleDepth,
                                      uint8_t ch)
{
  int8_t ret = 0;
...
//----------------------------------------------------------------------------------------------------------------------
int8_t
decodeResiduals (uint8_t warmup, uint8_t ch)
{

  int method = readUint (2);
  if (method >= 2)
    return ERR_FLAC_RESERVED_RESIDUAL_CODING; // Reserved residual coding
                                              // method
  ...
  if (m_blockSize % numPartitions != 0)
    return ERR_FLAC_WRONG_RICE_PARTITION_NR; // Error: Block size not divisible
                                             // by number of Rice partitions
      ...
      if (param < escapeParam)
        {
          for (int j = start; j < end; j++)
            {
              FLACsubFramesBuff->samplesBuffer[ch][j]
                  = readRiceSignedInt (param);
            }
        }
      else
        {
...
//----------------------------------------------------------------------------------------------------------------------
void
restoreLinearPrediction (uint8_t ch, uint8_t shift)
{
