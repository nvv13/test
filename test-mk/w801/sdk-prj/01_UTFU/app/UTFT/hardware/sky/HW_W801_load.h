...
word
UTFT_loadBitmap (int x, int y, int sx, int sy, char *filename)
{
  ...
  if (res_sd == FR_OK)
    {
      ...
      printf (
          "UTFT_loadBitmap:Open file successfully! Start reading data!\r\n");
#endif
      fnum = sizeof (buf_4x_line);
      if (strstr (filename, ".bmp") != NULL
          || strstr (filename, ".BMP") != NULL)
        { //надо пропустить заголовок
          res_sd
              = f_read (&fnew, buf_4x_line, sizeof (BITMAPFILEHEADER), &fnum);
          if (res_sd == FR_OK)
            {
...
#ifdef SERIAL_DEBUG
                  printf ("UTFT_loadBitmap:sizeof(BITMAPFILEHEADER)=%d, "
                          "aHead->bfOffBits=%d\r\n",
                          sizeof (BITMAPFILEHEADER), aHead->bfOffBits);
#endif
                  uint32_t
                      bmpImageoffset; // Start address of image data in file
                  bmpImageoffset = aHead->bfOffBits - 1; // Start of image data
                  res_sd = f_lseek (
                      &fnew,         /* [IN] File object */
                      bmpImageoffset /* [IN] Offset of file read/write pointer
                                        to be set */
                  );
#ifdef SERIAL_DEBUG
      ...
      while (fnum == (sizeof (buf_4x_line)))
        {
          res_sd = f_read (&fnew, buf_4x_line, sizeof (buf_4x_line), &fnum);
          if (res_sd == FR_OK)
            {
#ifdef SERIAL_DEBUG_ALL
                  ...
                  while (cp < fnum)
                    {
                      if (((fnum - cp) / 2) < (sx - cx))
                        {
                          UTFT_setXY (x + cx, y + cy,
                                      x + cx + ((fnum - cp) / 2) - 1, y + cy);
                          for (int i = (fnum - cp) - 2; i >= 0; i -= 2)
                            UTFT_LCD_Write_DATA (buf_4x_line[cp + i],
                                                 buf_4x_line[cp + i + 1]);
                          cx += ((fnum - cp) / 2);
                          cp = fnum;
                        }
      ...
      // close file
      f_close (&fnew);
      UTFT_setXY (0, 0, UTFT_getDisplayXSize () - 1,
                  UTFT_getDisplayYSize () - 1);
      sbi (P_CS, B_CS);
    }
...
#endif
                res_sd= f_read(&fnew, buf_4x_line, sizeof(BITMAPFILEHEADER),
&fnum); if(res_sd == FR_OK)
                        {
                        // Parse BMP header to get the information we need
                        PBITMAPFILEHEADER aHead=(PBITMAPFILEHEADER)buf_4x_line;
                        if(aHead->bfType== 0x4D42)
                                { // BMP file start signature check
                                uint32_t bmpImageoffset;        // Start
address of image data in file bmpImageoffset = aHead->bfOffBits; // Start of
image data res_sd = f_lseek ( &fnew,  // [IN] File object bmpImageoffset  //
[IN] Offset of file read/write pointer to be set
                                        );
#ifdef SERIAL_DEBUG_ALL
                                        printf ("f_lseek successfully!
%d\r\n",bmpImageoffset); #endif fnum=BEF_LEN_3X;
                                }
                        }
...
(((bnum-cp)/2)<(sx-cx))
                                                        {
                                                                UTFT_setXY(x+cx,
y+cy, x+cx+((bnum-cp)/2)-1, y+cy); for (int i=(bnum-cp)-2; i>=0; i-=2)
                                                        ...
                                                        {
                                                                UTFT_setXY(x+cx,
y+cy, x+sx-1, y+cy); for (int i=sx-cx-1; i>=0; i--)
