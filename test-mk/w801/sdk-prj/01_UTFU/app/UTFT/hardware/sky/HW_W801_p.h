...
sbi_RS (void)
{
  if (display_transfer_mode == 1 && sost_RS == 1)
    return;
  sbi (P_RS, B_RS);
}
static void
cbi_RS (void)
{
  if (display_transfer_mode == 1 && sost_RS == 0)
    return;
  cbi (P_RS, B_RS);
}
...
sbi_SDA (void)
{
  if (display_transfer_mode == 1 && sost_SDA == 1)
    return;
  sbi (P_SDA, B_SDA);
}
static void
cbi_SDA (void)
{
  if (display_transfer_mode == 1 && sost_SDA == 0)
    return;
  cbi (P_SDA, B_SDA);
}
...
pulse_low_SCL (void)
{
  ...
  if (B_SCL >= WM_IO_PB_00) // w801 chip, only two GPIO port, PA (GPIOA - 16
...
static void pulse_high(uint32_t* PORT_REG, u8 PIN)
{
 ...
 if(PIN==NO_GPIO_PIN)return;
        ...
        if (PIN >= WM_IO_PB_00) // w801 chip, only two GPIO port, PA (GPIOA -
16 bit) and PB (GPIOB - 32 bit), max power 12ma
        {
          pin    = PIN - WM_IO_PB_00;
          offset = TLS_IO_AB_OFFSET;
        }
...
enabled control reg from need pin
        ...
        tls_reg_write32(HR_GPIO_DATA + offset, reg | (1 << pin));// write Hi
from pin

        tls_reg_write32(HR_GPIO_DATA + offset, reg & (~(1 << pin)));// write
low from pin
