      ...
      for (;;)
        {
          ...
          if (fno.fattrib & AM_DIR)
            ...
            { /* It is a file. */
              ...
              if (strstr (FileName, "240x135") != NULL && strstr (FileName, ".jpg") != NULL)
               {
               UTFT_ADD_lcd_draw_jpeg (FileName, 0, 0);
               //UTFT_loadBitmap ( 0, 0, 240, 135,  FileName); // выводим на дисплей картинку
               UTFT_setFont (SmallFont);
               UTFT_setColor2 (VGA_FUCHSIA);
               UTFT_print (fno.fname, CENTER, 100, 0);
...
{
  printf ("user_app1_task start TFT01_14V89 135x240 SPI st7789v\n");

  // подключаем библиотеку UTFT

  // TFT01_47V89 - для ST7789v
  UTFT_UTFT (TFT01_14V89
...
*/
  ...
  while (1)
    { //
      ...
      UTFT_setColor2 (VGA_WHITE); // 320x172
      for(int i=2;i<30;i++){
      UTFT_drawRect (1, 1, i*8, i*4); 
      }
      ...
      UTFT_drawRoundRect (
          10, 10, 230,
          125); // Рисуем прямоугольник со скруглёнными углами (с
      ...
      UTFT_drawCircle (
          120, 67,
          60); // Рисуем окружность (с центром в точке x y  и радиусом r)

      UTFT_fillCircle (120, 67, 50); // Рисуем закрашенную окружность (с
