...
//          TFT32MEGA_2                        // если изображение на дисплее отображается зеркально, значит для инициализации Вашего дисплея, нужно указать не TFT32MEGA, а TFT32MEGA_2.
   ...
   UTFT_clrScr();                            // стираем всю информацию с дисплея
                                               //
for(;;){                                   //
   UTFT_fillScr2(VGA_RED   ); n_delay_ms(1000);    // заливаем дисплей красным,     ждём 1 секунду
   UTFT_fillScr2(VGA_GREEN ); n_delay_ms(1000);    // заливаем дисплей зелёным,     ждём 1 секунду
   UTFT_fillScr2(VGA_BLUE  ); n_delay_ms(1000);    // заливаем дисплей синим,       ждём 1 секунду
   UTFT_fillScr2(VGA_SILVER); n_delay_ms(1000);    // заливаем дисплей серебристым, ждём 1 секунду
   UTFT_fillScr2(VGA_MAROON); n_delay_ms(1000);    // заливаем дисплей бордовым,    ждём 1 секунду
   UTFT_fillScr2(VGA_NAVY  ); n_delay_ms(1000);    // заливаем дисплей тем. синим,  ждём 1 секунду
}                                              //
