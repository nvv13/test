...
static char *
scan_privacy_string (u8 privacy)
{
  char *sec;
...
wifi_scan_handler (void)
{
  ...
  for (i = 0; i < wsr->count; i++)
    {
      j = sprintf (buf1, "bssid:%02X%02X%02X%02X%02X%02X, ",
  ...
  for (i = 0; i < wsr->count; i++)
    {
      ...
      if (strlen ((const char *)ssid) > 0)
        {
          for (u8 ind = 0; ind < _u8_WIFI_ARR_COUNT; ind++)
            {
              // printf ("find ssid=[%s]\n",_aWIFI[ind*2]);
              if (strstr ((const char *)_aWIFI[ind * 2], (const char *)ssid)
                  != 0)
                {
      ...
      j += sprintf (buf1 + j, "\"%02x:%02x:%02x:%02x:%02x:%02x\", ",
                    bss_info->bssid[0], bss_info->bssid[1], bss_info->bssid[2],
                    bss_info->bssid[3], bss_info->bssid[4],
                    bss_info->bssid[5]);
...
static void
wifi_scan_format2_filter_handler (void)
{
  scan_done = 1;
  ...
  for (i = 0; i < wsr->count; i++)
    {
      ...
      if (bitmask & 0x1)
        {
          if ((specifiedinfo.specified_ssid_len != bss_info->ssid_len)
              || memcmp (bss_info->ssid, specifiedinfo.specified_ssid,
                         bss_info->ssid_len))
            {
              bss_info++;
              continue;
      ...
      if (bitmask & 0x2)
        {
          sprintf ((char *)bssid, "%02x:%02x:%02x:%02x:%02x:%02x",
                   bss_info->bssid[0], bss_info->bssid[1], bss_info->bssid[2],
                   bss_info->bssid[3], bss_info->bssid[4], bss_info->bssid[5]);
          if (strcmp ((const char *)bssid,
                      (const char *)specifiedinfo.specified_bssid))
            {
              bss_info++;
              continue;
            }
...
int
user_scan_specified (u8 *ssid, u8 *mac, u8 chan, u8 scan_type,
  ...
  if (min_interval == 0)
  ...
  else
    {
      ...
      if (max_interval >= min_interval)
        {
          wscan_param.scan_times
              = (max_interval + min_interval / 2) / min_interval;
        }
      else
        {
          wscan_param.scan_times = 1;
...
u8
scan_format2_demo (const char **aWIFI, const u8 u8_WIFI_ARR_COUNT)
{
...
/*specified Scan demo*/
int
scan_specified_demo (u8 *ssid, u8 *mac, u32 chan, u32 scan_type,
                     u32 min_interval, u32 max_interval)
{
