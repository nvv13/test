...
extern "C"
{
  ...
  /**
   * @brief Convert a @p hex value of the form 0x00RRGGBB to an RGB color
   * struct
   *
   * @note                the two most significant bytes of @p hex will be
   * ignored
   *
  ...
  /**
   * @brief Convert a hex color string of the form 'RRGGBB' to a color_rgb_t
   * struct
   *
  ...
  /**
   * @brief Convert a color_rgb_t struct to a hex color string of the form
   * 'RRGGBB'
   *
   ...
   *
   * @param[in] rgb       Input rgb color, that should be converted. Must be
   * NOT NULL
   * @param[out] inv_rgb  Output rgb color, result of the conversion. Must be
   * NOT NULL
   */
  static inline void
  color_rgb_invert (const color_rgb_t *rgb, color_rgb_t *inv_rgb)
  {
   ...
   *
   * @param[in] rgb       Input rgb color, that should be shifted. Must be NOT
   * NULL
   * @param[out] out      Output rgb color, result of the shift. Must be NOT
   * NULL
   * @param[in] shift     Amount by which the color components should be
   * shifted. May be positive (shift to left) or negative (shift to right).
   */
  static inline void
  color_rgb_shift (const color_rgb_t *rgb, color_rgb_t *out, int8_t shift)
  {
    if (shift > 0)
      {
        out->r = rgb->r << shift;
  ...
  /**
   * @brief Change the brightness of a RGB color by multiplying it with a set
   * factor.
   *
   * @pre                 ((rgb != NULL) && (out != NULL))
   *
   * @param[in] rgb       Input rgb color, that should be multiplied. Must be
   * NOT NULL
   * @param[out] out      Output rgb color. Must be NOT NULL
   * @param[in] level     New brightness level. 255 = Full Brightness, 0 = Off.
   */
  static inline void
  color_rgb_set_brightness (const color_rgb_t *rgb, color_rgb_t *out,
                            uint8_t level)
  {
   ...
   *
   * @note                Complementary color calculation according to adobe
   * illustator calculations. See
   ...
   * @param[out] comp_rgb Output rgb color, result of the complementary color
   * calculation. Must be NOT NULL
   */
