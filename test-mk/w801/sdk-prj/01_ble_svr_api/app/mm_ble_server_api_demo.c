...
/** Misc. */
extern void print_bytes(const uint8_t *bytes, int len);
extern void print_string(const int8_t *bytes, int len);

//extern void print_mbuf(const struct os_mbuf *om);
extern char *addr_str(const void *addr);
//extern void print_uuid(const ble_uuid_t *uuid);
extern void print_conn_desc(const struct ble_gap_conn_desc *desc);
extern void print_adv_fields(const struct ble_hs_adv_fields *fields);
...
int mm_ble_server_api_demo_adv(bool enable)
{
    int rc;
...
int
mm_ble_server_demo_gatt_svr_init(void)
{
    int rc;
...
static void ble_server_indication_sent_cb(int conn_id, int status)
{
    ...
    if(g_ble_uart_output_fptr == NULL)
    {
        ...
        ss++;
        if(ss > 0xFE) ss = 0x00;
    	mls_ble_server_demo_api_send_msg(g_ind_data, g_mtu); 
    ...
    }else
    {
        ...
        if(len)
        {
            ...
            ret = mls_ble_server_demo_api_send_msg(g_ind_data, len); 
...
static void mm_ble_server_demo_start_indicate(void *arg)
{
    int len;
    int rc;
    ...
    if(g_ble_uart_output_fptr == NULL)
    {
	    rc = mls_ble_server_demo_api_send_msg(g_ind_data, g_mtu);
    ...
    }else
    {
        ...
        {
            tls_ble_uart_buffer_peek(g_ind_data, len);
            status = mls_ble_server_demo_api_send_msg(g_ind_data, g_mtu);
            ...
            }else
            {
               ...
               tls_bt_async_proc_func(mm_ble_server_demo_start_indicate,(void*)g_ble_demo_indicate_enable,1000);
            }
        }
    }    
}
...
static void mm_ble_server_demo_conn_param_update_slave()
{
	int rc;
    ...
    switch(event->type)
    {
        ...
        case BLE_GAP_EVENT_SUBSCRIBE:
            ...
            if(g_ble_demo_indicate_enable)
            {
                ...
                if(g_ble_uart_output_fptr)
                {
                    tls_bt_async_proc_func(mm_ble_server_demo_conn_param_update_slave, NULL, 30);
                }
                tls_bt_async_proc_func(mm_ble_server_demo_start_indicate,(void*)g_ble_demo_indicate_enable, 30);
            }else
            {
                if(g_ble_server_state != BLE_SERVER_MODE_EXITING)
                {
...
int mls_ble_server_demo_api_init(tls_ble_output_func_ptr output_func_ptr)
{
    int rc = BLE_HS_EAPP;
    ...
    if(g_ble_server_state == BLE_SERVER_MODE_IDLE)
    {
        ...
        if(rc != 0)
        {
            TLS_BT_APPL_TRACE_ERROR("mls_ble_server_demo_api_init failed rc=%d\r\n", rc);
            return rc;
        }

        //step 1: config/adding  the services
        rc = mm_ble_server_demo_gatt_svr_init();
		...
		{	
			...
			TLS_BT_APPL_TRACE_DEBUG("### mm_ble_server_api_demo_init \r\n");
            ...
            rc = mm_ble_server_api_demo_adv(true); 
		...
		{
			TLS_BT_APPL_TRACE_ERROR("### mm_ble_server_api_demo_init failed(rc=%d)\r\n", rc);
		}
    }
    else
    {
    	TLS_BT_APPL_TRACE_WARNING("mm_ble_server_api_demo_init registered\r\n");
        rc = BLE_HS_EALREADY;
    }
	
	return rc;
}
int mls_ble_server_demo_api_deinit()
{
   int rc = BLE_HS_EAPP;
...
uint32_t mls_ble_server_demo_api_get_mtu()
{
    return g_mtu;
}
int mls_ble_server_demo_api_send_msg(uint8_t *data, int data_len)
{
    int rc;
    struct os_mbuf *om;
