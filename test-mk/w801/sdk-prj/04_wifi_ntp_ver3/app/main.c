...
static u16 i_dreb2=0; // от дребезга
       u16 i_pos_dreb2=500;
static int i_rotar=10;
static u16 i_rotar_zero =0;
static u16 i_rotar_one  =0;
...
if(i_rotar_zero>0 ||  i_rotar_one>0) //покрутили
 {
 if(i_dreb2++>i_pos_dreb2)//можно отсчитывать временной интервал
   {
   if(i_rotar_zero>i_rotar_one)
    i_rotar--;
    else
    i_rotar++;
   i_dreb2=0; // от дребезга
   i_rotar_zero=0;
   i_rotar_one =0;
   i_rotar_value=1;
   } 
...
#define PA1_ISR_IO_FALLING_SW		WM_IO_PA_01
static void demo_gpio_isr_callback(void *context)
{

	u16 ret = tls_get_gpio_irq_status(PA1_ISR_IO_FALLING_SW);
	//printf("\nint flag =%d\n",ret);
	if(ret)
	{
  		 tls_clr_gpio_irq_status(PA1_ISR_IO_FALLING_SW);
...
#define PA2_DT		WM_IO_PA_02
#define PA3_ISR_IO_FALLING_CLK		WM_IO_PA_03
static void pa3_isr_callback(void *context)
{

	u16 ret = tls_get_gpio_irq_status(PA3_ISR_IO_FALLING_CLK);
	if(ret)
	{
  		tls_clr_gpio_irq_status(PA3_ISR_IO_FALLING_CLK);
                if(i_dreb2==0)
                 {
                 if(tls_gpio_read(PA2_DT))
	           i_rotar_one++;
                   else
	           i_rotar_zero++;        
                 }
...
void demo_console_task(void *sdata)
{
   ...
   //
   tls_gpio_cfg(PA1_ISR_IO_FALLING_SW, WM_GPIO_DIR_INPUT, WM_GPIO_ATTR_FLOATING);
   tls_gpio_isr_register(PA1_ISR_IO_FALLING_SW, demo_gpio_isr_callback, NULL);
   tls_gpio_irq_enable(PA1_ISR_IO_FALLING_SW, WM_GPIO_IRQ_TRIG_FALLING_EDGE);
   printf("\nPA1_ISR_IO_FALLING_SW gpio %d FALLING isr\n",PA1_ISR_IO_FALLING_SW);
   //
   tls_gpio_cfg(PA3_ISR_IO_FALLING_CLK, WM_GPIO_DIR_INPUT, WM_GPIO_ATTR_FLOATING);
   tls_gpio_isr_register(PA3_ISR_IO_FALLING_CLK, pa3_isr_callback, NULL);
   tls_gpio_irq_enable(PA3_ISR_IO_FALLING_CLK, WM_GPIO_IRQ_TRIG_FALLING_EDGE);
   printf("\nPA3_ISR_IO_FALLING_CLK gpio %d FALLING isr\n",PA3_ISR_IO_FALLING_CLK);
   //
   tls_gpio_cfg(PA2_DT, WM_GPIO_DIR_INPUT, WM_GPIO_ATTR_FLOATING);
   //
