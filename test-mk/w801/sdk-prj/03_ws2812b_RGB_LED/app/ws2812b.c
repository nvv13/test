...
static inline void shift(const ws2812b_t *dev, uint32_t data)
{
    for (int i = 23; i >= 0; i--) {
        if( ((data >> i) & 0x01) )
        {//1
        tls_gpio_write(dev->data_pin, 1);//gpio_write(dev->data_pin, 0x01);//T1H 1 code ,high voltage time 0.85us ±150ns
        tls_gpio_write(dev->data_pin, 1);//gpio_write(dev->data_pin, 0x01);//T1H 1 code ,high voltage time 0.85us ±150ns
        tls_gpio_write(dev->data_pin, 1);//gpio_write(dev->data_pin, 0x01);//T1H 1 code ,high voltage time 0.85us ±150ns
        tls_gpio_write(dev->data_pin, 0);//gpio_write(dev->data_pin, 0x00);//T1L 1 code ,low voltage time 0.4us ±150ns
        }
        else
        {//0
        tls_gpio_write(dev->data_pin, 1);//gpio_write(dev->data_pin, 0x01);//T0H 0 code ,high voltage time 0.4us ±150ns
        tls_gpio_write(dev->data_pin, 0);//gpio_write(dev->data_pin, 0x00);//T0L 0 code , low voltage time 0.85us ±150ns
