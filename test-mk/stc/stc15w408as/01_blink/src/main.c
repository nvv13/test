...
// >>	bitwise shift right; moves the bits to the right, discards the far
// right bit and if unsigned assigns 0 to the left most bit, otherwise sign
// extends.
// <<	bitwise shift left; moves the bits to the left, it discards the far
// left bit and assigns 0 to the right most bit.
...
setup (void)
{
  ...
  0 0 quasi_bidirectional(traditional 8051 I/O port output) ,
      ...
      Because of manufactured error, the actual pull-up current is 270uA ~
  150uA это и вход (можно читать что на входе), и выход, открытый коллетор с
  резистором на питание (ток примерно 270мКа), если вниз на 0 -то транзистор
  тянет до 20ma

  0 1 push-pull output(strong pull-up output,current can be up to 20mA,
  resistors need to be added to restrict current выход, тяни-толкай режим (до
  20ma ток)

  1 0 input-only (high-impedance )
      как вход,

  1 1 Open Drain,internal pull-up resistors should be disabled and external
  pull-up resistors need to join. открытый коллектор, транзистор тянет до 20ma
  на 0, если что, нуждаеться во внешнем резисторе на питание

  *все порты, по умолчанию, в тежиме 0 0 quasi_bidirectional, однако,
  P1.7/XLAT1 и P1.6/XLAT2 могут использовать внешний кристал МК при старте
  проверяет, также при старте, проверяется режим "прошивки" кристала...
  ...
  while (1)
    {
      ...
      P1_2 = 0; // out 0 pin 2 port 1
                //при записи в память, по адресу P1_2, происходит запись во 2
                //бит порта 1, это так называемое,
                // проецирование(отображение) памяти на бит порта, применяеться
                // в некоторых МК
      delay_ms (i_my_deley); //
      ...
      // 	sfr P1          =   0x90;         // (sfr - спец. функция
      // регистр) 	sbit P12        =   P1^2;
      //  т.е. записывает в ячейку  0x90^2=0x92 специального адр. пространства
      //  __sbit
