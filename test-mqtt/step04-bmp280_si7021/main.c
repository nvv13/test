...
static int _cmd_readtemp(int argc, char **argv)
{
    ...
    switch (bmx280_init(&dev280, &bmx280_params[0])) {
        case BMX280_ERR_BUS:
...
//    puts("\n+--------Starting Measurements--------+");
        ...
        int16_t temperature = bmx280_read_temperature(&dev280);
        uint32_t pressure = bmx280_read_pressure(&dev280);
#if defined(MODULE_BME280_SPI) || defined(MODULE_BME280_I2C)
        uint16_t humidity = bme280_read_humidity(&dev280);
#endif
        ...
        //printf("   Pressure [Pa]: %" PRIu32 "\n", pressure);
           ...
           char* a1[]={"pub","/temp2",str_t};
           if(_cmd_pub(3, a1))
            {
            char* a0[]={"con","192.168.1.47","1883","cl1"};
           _cmd_con(4, a0);
            };
     xtimer_sleep(1);
           ...
           //len = sprintf(str_t,"%" PRIu32 "", pressure / 133);
           str_t[len] = 0;
           char* a2[]={"pub","/pressure2",str_t};
           _cmd_pub(3, a2);
     xtimer_sleep(1);
    ...
    int16_t temperature70;
    uint16_t humidity70;

    if (si70xx_init(&dev70, &si70xx_params[0]) == 0) {
        puts("[OK] si7021");
    si70xx_get_both(&dev70, &humidity70, &temperature70);
           
            str_t[0]=0;
            len = sprintf(str_t,"%d.%02d",
                   temperature70 / 100,
                   temperature70 % 100);
            str_t[len] = 0;

           char* a4[]={"pub","/temp1",str_t};
           _cmd_pub(3, a4);
     xtimer_sleep(1);
...
static const shell_command_t shell_commands[] =
{
    ...
    { "temp",   "read temp ds18b20",                  _cmd_readtemp  },
    { NULL,     NULL,                                 NULL        }
};
...
int main(void)
{
    ...
    printf("dig_T1: %u\n", dev280.calibration.dig_T1);
    printf("dig_T2: %i\n", dev280.calibration.dig_T2);
    printf("dig_T3: %i\n", dev280.calibration.dig_T3);

    printf("dig_P1: %u\n", dev280.calibration.dig_P1);
    printf("dig_P2: %i\n", dev280.calibration.dig_P2);
    printf("dig_P3: %i\n", dev280.calibration.dig_P3);
    printf("dig_P4: %i\n", dev280.calibration.dig_P4);
    printf("dig_P5: %i\n", dev280.calibration.dig_P5);
    printf("dig_P6: %i\n", dev280.calibration.dig_P6);
    printf("dig_P7: %i\n", dev280.calibration.dig_P7);
    printf("dig_P8: %i\n", dev280.calibration.dig_P8);
    printf("dig_P9: %i\n", dev280.calibration.dig_P9);

#if defined(MODULE_BME280_SPI) || defined(MODULE_BME280_I2C)
    printf("dig_H1: %u\n", dev280.calibration.dig_H1);
    printf("dig_H2: %i\n", dev280.calibration.dig_H2);
    printf("dig_H3: %i\n", dev280.calibration.dig_H3);
    printf("dig_H4: %i\n", dev280.calibration.dig_H4);
    printf("dig_H5: %i\n", dev280.calibration.dig_H5);
    printf("dig_H6: %i\n", dev280.calibration.dig_H6);
#endif
