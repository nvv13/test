...
//        printf("   Pressure [Pa]: %" PRIu32 "\n", pressure);
           ...
           str_t2[0]=0;
           len = sprintf(str_t2,"%d.%02d", pressure / 133, pressure % 133);
           str_t2[len] = 0;
           a_pressure2[2]=str_t2;
...
if(b_lcd_ok)
 ...
 hd44780_print(&lcd_dev, "gaz=");
 hd44780_print(&lcd_dev, str_t);

 hd44780_set_cursor(&lcd_dev, 0, 1);
 hd44780_print(&lcd_dev, "temperature=");
 hd44780_print(&lcd_dev, str_temp);

 hd44780_set_cursor(&lcd_dev, 0, 2);
 hd44780_print(&lcd_dev, "humidity=");
 hd44780_print(&lcd_dev, str_hum);

 hd44780_set_cursor(&lcd_dev, 0, 3);
 hd44780_print(&lcd_dev, "pressure=");
 hd44780_print(&lcd_dev, str_t2);
 }
...
int main(void)
{
    ...
    if (hd44780_init(&lcd_dev, &hd44780_params[0]) != 0) 
     {
     puts("hd44780 init OK\n");
     b_lcd_ok=true;
     }
     else
     puts("hd44780 init ERROR\n");

    adc_line0_init=adc_init(adc_line0);
    if(adc_line0_init==0)
     {
     puts("ADC init OK\n");
     print_lcd("ADC init OK");
     }
     else
     puts("ADC init ERROR\n");
